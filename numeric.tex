...
\begin{equation} \cos \upsilon = \cos \upsilon = \cos(\upsilon+2\pi) \quad  \forall \upsilon\in\mathbb{C} \label{eq_}\end{equation}
\begin{equation} \upsilon^2 = (-\upsilon)^2 \quad \forall \upsilon\in\mathbb{C} \label{eq_}\end{equation}
Although arccosine and square root are defined clearly and uniquely in mathematics, physical interpretation of data requires one to determine 
...
It is assumed that for any realistic structure of nonzero losses, the transmission never passes exactly through the complex zero, and the arccosine argument from Eq. (\ref{eq_Neff})
\begin{equation} \upsilon(f) = \frac{1-r^2+t^2}{2t},   \label{eq_upsilon}\end{equation}
is a continuous complex function. Any discontinuities in the retrieved spectra of $\Neff'(f)$ may therefore only arise at the discontinuities of the arccosine function in the complex plane.

To remove these mathematical artifacts it is necessary to identify the \textit{branch cuts} of arccosine in the complex plane, as illustrated in the left subplots of Fig. \ref{fg_arccos}a,b. They are two, on the left and right rays at the real axis. Different measures must be taken for the sign and branch index $m(f)$ to ensure continuity:
\begin{enumerate}
\item{
...
\item{
 ...
 \end{enumerate}
The correct reconstruction of the arccosine therefore only requires to compute the integer-valued branch along with the sign, both being functions of frequency. 
...
Continuous computation of arccosine and square root are implemented in Ref. \cite[effparam.py file]{dominec2014_meep_metamaterials}.

%}}}
\paragraph{Initial branch and sign choices}
By using 
\paragraph{Weakness of the method used}
