...
%}}}
\paragraph{Limitations of the computation of continuous effective parameters}%{{{
...
\begin{enumerate}
 ...
 In the view of the author of this thesis, also the retrieved $\Neff$ in Fig. 7c of Ref. \ref[p. 036617-9]{smith2005electromagnetic} allows reasonable interpretation if and only if the structure is symmetric. 
			 }
\end{enumerate}

Another important limitation of this method, however, comes from its principle of retrieving the wavenumber at a given frequency. For periodic structure which exhibits strong enough spatial dispersion, more than one wavenumber exist at a single frequency as shown in Figs. \ref{fg_dcll_nl}. For any frequency from such a problematic range, the detected effective parameters depend on the unknown ratio of energy coupled to either of the waves. Therefore, the method is inapplicable for structures with a strong spatial dispersion. This effect is illustrated in the Results section (e.g. in Fig. \ref{fg_cdh2}).

%}}}

\subsection{Current-driven homogenisation} 
When more than one wavevector $\KK$ corresponds to a given frequency,  %{{{
a fundamentally different retrieval method must be used, for which the wavevector $\KK$ becomes the input, and the corresponding frequencies $\omega_{1\ldots\infty}(K)$  at the dispersion curves are returned as the output. 

This section describes how a simulation can be setup to use a single wavevector $\KK$ only, and an efficient way of reconstructing the dispersion curves from a scan through different wavevector magnitudes. 
%}}}
...
If the source amplitude had an infinite duration in time, it would form a perfect rectangle-like function in its spectrum. However, clipping the temporal profile of the source, results in artifacts in the spectrum by the same mechanism as described on page \ref{convolringing}.
...
%}}}
\paragraph{Volume-averaging monitor and identification of the dispersion curves}%{{{
