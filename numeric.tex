...
This section describes the numerical methods used for preparation of this thesis. Major accent is put on the finite-difference time-domain method, since it was used for most simulations. Several important observations are discussed which are probably not found elsewhere.

%% XXX
Second, this chapter briefly mentions the plane-wave expansion method. 
...
Other numerical methods than these two are used in the literature and should be briefly listed.  
...
%% TODO Boundary-element methods, Fourier modal method, 
...
%\end{enumerate}
%}}}
%\paragraph{Other numerical methods} %{{{
%}}}
...
%}}}
\paragraph{Search of the correct solution} %{{{
...
\begin{enumerate}
 ...
 \begin{quote} \textit{ so different are the two solutions for $\Zeff$ for the asymmetric structure that in general the assignment of values of $\eeff$ and $\meff$ to the composite becomes counterproductive} \end{quote}
...
This section describes how a simulation can be setup to enforce a given wavevector $\KK$ only, and an efficient way of reconstructing the dispersion curves from a scan through different wavevector magnitudes. For this thesis it was implemented in its simplest form. Its more elaborate implementation is described in Refs. \cite{silveirinha2007metamaterial}, \cite{fietz2010current} and \cite{fietz2011homogenization} and enables to recover all 36 parameters that describe the influence of the fields ($E_x$, $E_y$, $E_z$, $H_x$, $H_y$, $H_z$) to the displacements ($D_x$, $D_y$, $D_z$, $B_x$, $B_y$, $B_z$), including anisotropy and bianisotropy.

%}}}
\paragraph{Bloch-periodic boundaries for arbitrary wave vector} %{{{
In the method described here, the unit cell is simulated as being placed in an infinite lattice, neighbouring with the same cells of size $a$ in all three dimensions. To emulate the infinite lattice in a simulation of a single cell, all the faces of the unit cell are set to copy the field from the opposite face.

Exact copying of the fields from one side to another would require the wavevector of the Bloch wave to be strictly $K_{xyz} \in 2\pi m/a_{xyz}$, which is, however, the known condition for a photonic band gap. Since we are also interested in computing the wavenumber inside photonic bands, the periodic boundaries have to allow for an arbitrary phase shift, before the fields are copied:
...
Positive phase advance is applied when copying the fields parallel to the x-axis, and negative phase retardation when simultaneously copying the fields in the opposite direction:
...
In order to excite the simulation volume with a single wavevector $\KK$ which complies to the Bloch-periodic boundary conditions imposed, the source volume must expand over the whole unit cell and acquire the same harmonic modulation of its complex amplitude. While this task appears impossible by experimental techniques, it is straightforward in the FDTD simulation. The source is typically designed to be a complex-valued electric current with a given amplitude:
...
Excitation of the structure with this kind of the source gave the \textit{current-driven homogenisation} (CDH) its name. Unlike the scattering-parameters method, in CDH the source volume coincides with the entire unit cell volume. An attempt of visualisation of this minimalistic simulation setup is in Fig. \ref{fg_fdtd_cdh}. One example of the real part of the source amplitude is sketched along the unit cell edge, as the green-filled curve.
\begin{figure}[h] \centering \caption{Current-driven homogenisation setup consists of a single unit cell with all faces set to be Bloch-periodic.} \includegraphics[width=.6\textwidth]{img/cdh_geometry.pdf}  \label{fg_fdtd_cdh} \end{figure} %% 
%}}}
\paragraph{Temporal profile of the source}%{{{
With the wavevector restricted to a single given value, it is necessary to excite and detect as many corresponding modes of the structure as possible. Therefore, a short and broadband temporal source profile could be copied from the scattering parameter method.. 

Unlike the scattering-parameters method, CDH can not easily detect the spectral profile of the exciting field, and the retrieved fields can not be normalized against it. To maintain an approximately flat source amplitude over a wide part of the specturm, a nontrivial temporal shape of the source was designed, tested and submitted to the MEEP simulation developer site online.
\begin{equation} 
...
\begin{equation} \mathrm{Si}(t) = \int_0^t \frac{\sin \tau}{\tau} d\tau, \label{eq_Si}\end{equation}
	The electric field radiated by a source is a temporal derivative of the current $\mathbf{j}(t)$ \cite{ab-initio}, therefore the sine-integral must be used.

If the source amplitude had an infinite duration in time, it would form a perfect rectangle-like function in its spectrum. However, clipping the temporal profile of the source, results in artifacts in the spectrum by the same mechanism as described on page \pageref{convolringing}.
...
\begin{split} 
	...
	w_{BN} :=\;& 0 \quad \text{otherwise.}
\end{split} 
\label{eq_wBN}\end{equation} %% TODO verify again
Since it has a finite support, but particularly fast decaying wings in its spectrum, it ensures relatively steep edges of the nearly rectangular spectral profile of the source.

%}}}
\paragraph{Field monitor and identification of the dispersion curves}%{{{
The detection of the fields was defined in a way similar to the definition of the source. In each timestep, the electric field $E_x(\rr,t)$ was sampled in multiple points through the unit cell. As each of the points could have a different phase, the values were divided by $e^{-\ii \KK\cdot \rr}$ before they were averaged.

After the simulation ends, the single record of cell-averaged $E_x$ is processed to identify the set of frequencies $\omega_m(\KK)$at which $E_x$ exhibited a ringdown. If the cell is empty vacuum, the source with wavevector $\KK$ can only excite a wave at the frequency $\omega = Kc$. All other frequencies are off the dispersion curve (known also as the light line in vacuum) and although the source can create temporary evanescent fields at these frequencies, their energy is returned back to the source since there is no solution of the Maxwell equations for  

The same holds for any kind of structure placed in the unit cell: the energy persists to the end of the simulation only at frequencies $\omega_m(\KK)$ that lie at one of corresponding dispersion curves. 

% TODO

Harmonic inversion was used based on the FDM algorithm \cite{mandelshtam1997harmonic}.
...
\end{enumerate}
In spite of these limitations, own simplified implementation of CDH remains an important complement to the s-parameter method. For selected structures, the numerical results are compared later.
