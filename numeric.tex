...
% LIST: 1) averaging of the fields, 2) 

\paragraph{Averaging of the fields} 
In contrast with the scattering parameters method, where $\Neff(f)$ and $\Zeff(f)$ were retrieved first using the amplitudes  of the reflectance and transmittance, and the local effective permittivity $\eeff(f)$ and permeability $\meff(f)$ were computed later, using Eqs. (\ref{eq_dispeqN}) and (\ref{eq_Z}). The local values of $\eeff(f)$ and $\meff(f)$  computed by the scattering parameter method are considered valid only when $|\Neff'| \ll 2\pi/a$, i.e., when the unit cell size $a$ is much less than the Bloch wavelength.
