%.. key originalities with respect to the state of the art in this field


%%% Hole : Conclusion - what to put here?
%% NOTES: What WAS discussed in the thesis and is special:
%  * THEORY aims to present an accessible introduction to the electrodynamics of periodic structures. 
%		including spatial dispersion to account for the material properties depending not only on the frequency, but also on the magnitude of the wave vector.
%  * historical notes 
% 	(the base line is that many concepts are not as novel as they may appear, and that modern papers
%	often fail to cite the author of the original idea,  questioning the supposed novelty and practical impact of MMs)
%    the historical review suggests that the the unification
%		of MM and PhC concepts is undergoing in the last two decades
%	 in an attempt for clarification of these two terms further 
...
%	* viability of the notion of "information velocity"   


%  * EXPERIMENTAL we developed an original apparatus for accurate sieving
...
%  * NUMERICAL provide the simulation environment which facilitates correct computations,  
...
%	- pointed out the conditions for observation of zero-width photonic bands (e.g. Fig. \ref{fg_cdh2}b) and zero-width photonic band gaps (e.g. Fig.)
...
In line with the quote from  % TODO 
...
%		In addition, the MM-related books available are often composed of ordinary papers, unrelated to each other and not covering all necessary theory, thus hardly preparing the reader to study the literature. % and instead of clear rigorous development of the theory, the books rather repeat nebulous phrases...
%
