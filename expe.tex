...
\begin{enumerate}
\item{Finite-difference time-domain (FDTD) is a time-marching method that describes the structure on an uniform grid, allowing to compute very general electromagnetic problems. Usually a short-time, broadband pulse excitation is used, which allows to resolve whole spectrum at once. The actual setup and data processing can vary according to the problem being solved. Two fundamentally different setups are described in the following chapter.} 
\item{Finite-difference frequency-domain (FDFD) method uses the same discretisation as FDTD, but instead of time-marching, it searches for the field eigenmode at one given frequency by means of numerical optimisation. It is favorable e.g. for mode analysis in complicated structures.  } 
...
\item{Transfer-matrix method (TMM), also allows to find dispersion curves. Although it can be extended to two or three dimensions, most popular is its one-dimensional implementation, as it is so simple to implement that it can even give an analytical insight into the behaviour of planar structures.  } 
\end{enumerate}
...
The finite difference methods, FDTD and FDFD, can be employed to wide variety of electromagnetic problems. Different ways exist to study the behaviour of a periodic structures. 
\subsection{Scattering parameters} % TODO
\paragraph{Principle} %{{{
The scattering parameter retrieval, also known as \textit{s-parameter} or \textit{Nichols-Ross-Weir} method, is probably the most common way to study periodic structures. It is based on the fact that one can easily compute the transmission $t$ and reflection $r$ from a slab of any material, provided its index of refraction $N$ and wave impedance $Z$ are given, along with the slab thickness $d$. The formulas can be inverted, so that one finds such (effective) parameters $\Neff$ and $\Zeff$ that give expected $r,t$ for a known layer thickness. 

Mathematically, this can always be done. The problem is opposite: the solutions have infinitely many branches. Aside from this, the physical interpretation of the data can get even more tricky when effective parameters are assigned to an inhomogeneous slab. Different problems of this method are resolved and discussed in the following. Even though, this method has the great advantage of being easy to realize experimentally.
...
The impact for the s-parameters method of the near-field reaching out of a structure is twofold:
\begin{enumerate}
...
The second one can be resolved by adding a sufficient \textit{padding} between the metamaterial cell and the monitor plane. As the evanescent field decays exponentially, usually less than half of the unit cell size is sufficient to suppress all artifacts from near-field detection. The wave propagating in air only gets an additional phase offset, that can be easily compensated after the simulation. } 
 \end{enumerate}
