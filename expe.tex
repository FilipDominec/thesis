...
\begin{figure}[t] \caption{Permittivity plot for titanium dioxide (rutile), for an ordinary ray. The above plot has linear vertical scale, while the bottom plot displays the same quantity using the scale that is $-\log(-\varepsilon_r)$ for $\varepsilon_r<-1$; linear for $-1<\varepsilon_r<1$ and $\log(\varepsilon_r)$ for $\varepsilon_r > 1$. The second approach is more suitable for plotting permittivity in the following discussion.} \label{fg_tio2eps} \centering 
	\includegraphics[width=12cm]{img/epsilon_TiO2_linear.pdf}
	\includegraphics[width=12cm]{img/epsilon_TiO2_symlog.pdf}
\end{figure}
...
The region forbidden by the stability rule is yellow shaded (above $f_c = 95$ THz and below $\varepsilon_r < 0.75$). The exact model for TiO$_{2}$ (green line) would be definitely unstable due to violating both stability conditions. The numerically stable model for frequency range of interest up to ca. 10 THz (black line) has all high-frequency oscillators substituted by increasing the value of $\varepsilon_{r\infty}$. Solid and dashed lines are real and imaginary parts, respectively. } \label{fg_tio2eps_stripped} \centering 
	\includegraphics[width=12cm]{img/epsilon_TiO2stripped_symlog.pdf}
% full:
      % self.eps = 1
      % self.pol = [
...
\begin{figure}[ht] \caption{Permittivity and conductivity plot for gold; vertical scale and the yellow region forbidden by the stability rules are the same as in Fig. \ref{fg_tio2eps_stripped}. The exact model of gold % TODO \cite{palik} 
(red) is compared to the lossy Drude model with Lorentz oscillators substituted by $\varepsilon_r$ (blue), and by the Drude model with zero scattering frequency (black). The bottom plot shows the conductivity of these three models as given by Eq. (\ref{eq_drude_sigma}).}
\label{fg_Au_models} \centering 
	\includegraphics[width=12cm]{img/epsilon_Au_models.pdf}
\end{figure}
...
Simulations in the optical range are relatively safe in terms of numerical stability. As illustrated above, a reasonable resolution of $\Delta x = 50$ nm ensures the critical frequency $f_c$ is far above the plasma frequency of metals or other conductors. , and no changes are usually needed to be made in the Drude model, though the simulation may run faster when one or more Lorentz terms are omitted (cf. red and blue curves in Fig. \ref{fg_Au_models}).
...
Realistic simulations of metals at lower resolutions however require taking measures to ensure stability, as the critical frequency $f_c$ is reduced below the plasma frequency of most metals when $\Delta x \gtrsim 100$ nm. % TODO check this
A trivial approach is to drastically reduce the Courant factor from $S=0.5$ to $0.01$ or even less, so that $f_c$ remains above plasma frequency. Although this reliably avoids the instability, it does so at the expense of scaling the computational time. As a general rule, lower resolution is typically chosen for larger structures, where also all investigated processes accordingly happen on a longer timescale.
