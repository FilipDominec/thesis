...
A similar sample used in experiment consisted of bars from strontium titanate (SrTiO$_{3}$) with  rectangular cross-section (see Fig. \ref{fg_STO_bar_geom}), 
...
The meshes had two purposes -- either as sieves used in the second sieving apparatus described above, or as metamaterial samples known as \textit{fishnets}, which are predicted to possibly have negative index of refraction (see Sect. \ref{sect_fishnet}). 
The femtosecond laser used was different from that serving as the femtosecond source for terahertz spectroscopy (Sect. \ref{sect_tdts}). Sufficient energy in the pulse power was obtained from the \textit{Spectra Physics Spitfire Ace} multipass amplifier, with \todo{}
The output beam had to be attenuated to obtain finer cut at the expense of slightly slower speed. 



\begin{figure}[ht] \caption{\textbf{(a)} Laser micromachining of the steel sheet, and \textbf{(b)} the resulting sieves} \label{fg_microfab} \centering 
...
\begin{figure} \caption{Mechanically drilled fishnet}  \centering \includegraphics[width=10cm]{img/fishnet.pdf} \end{figure} \clearpage
