...
\begin{equation} f_c := \frac{1}{\pi \, \Delta t} \equiv \frac{c}{\pi\, S\, \Delta x} \label{eq_}\end{equation}
...
In practice, a slightly more conservative choice is made that provides safe margin for numerical imprecision: 
...
which can be reformulated that  
}
%}}}
...
It is therefore advisable to weed the oscillators to a minimum acceptable number necessary, and to describe the material within some frequency range of interest % TODO  define range of interest
only:
\begin{enumerate}
 \item{
%Often the first step in processing FDTD results lies in converting them to frequency domain via Fourier transform. 
The upper bound for the range of interesting frequencies is limited by numerical stability as stated above. Even more often, a more strict limit is given by the spatial resolution of the simulation: In a high-permittivity material, high frequencies correspond to wavelengths similar to the voxel size (or even smaller than it), leading to significant inaccuracy. 
...
If some of the high-frequency oscillators introduces significant dispersion or losses to the frequency range of interest, it it should not be eliminated this way. This concerns oscillators that are close enough to the frequency range of interest, so they usually can be kept without causing instability.
} 
 \item{
Very low frequencies, corresponding to wavelengths much greater than the whole simulation volume, are theoretically accessible with a long-enough FDTD simulation, but the frequency range of interest usually does not reach zero frequency, mostly for practical reasons. If needed, the low-frequency phenomena can often be computed more efficiently in a separate simulation with a lower resolution or even with different numerical methods.

The oscillators at frequencies too low can therefore be omitted without any change to the behaviour within the frequency range of interest. One important exception is the low-frequency oscillator that defines conductive behaviour, as described below.
} 
 \end{enumerate}
%}}}
...
%If a right frequency dependence of $\varepsilon_r(\omega)$ is chosen, conductivity can also be expressed by the local model of permittivity, and vice versa. 
...
%}}}
\paragraph{Defining metals for high-resolution simulations} %{{{
% TODO 
% as a high-resolution simulation we denote ... one that makes f_c above plasma ferquency 
%  
%  
%}}}
\paragraph{Defining resistive metals for stable low-resolution simulations} %{{{
Simulations in the optical range are relatively safe in terms of numerical stability. As illustrated above, a reasonable resolution of $\Delta x = 50$ nm ensures the critical frequency $f_c$ is far above the plasma frequency of metals or other conductors, and no changes are usually needed to be made to the Drude model. 
...
Realistic simulations of metals at lower resolutions however require taking measures to ensure stability, as the critical frequency $f_p$ is reduced below the plasma frequency of most metals when $\Delta x \geq 100$ nm. % TODO check this
A naive approach is to drastically reduce the Courant factor so that $f_c$ remains high enough as in high-resolution simulations. Although this reliably avoids the instability, it does so at the price of scaling the computational time. As a general rule, lower resolution is typically chosen for larger structures, where also all investigated processes accordingly happen on a longer timescale.
