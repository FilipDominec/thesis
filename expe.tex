...
\add{
Electromagnetic waves in the terahertz range are radiated when charged particles are subject to fast-enough acceleration at the picosecond scale. The processes may be sorted according to the medium in which this occurs and the origin of the force causing the acceleration.  % There are numerous ways to generate  

\paragraph{Kinetic energy of electron beam}
One class of devices uses the kinetic energy of an electron beam propagating in vacuum; the acceleration of the electrons is caused by transverse magnetic field. The simplest example is thecyclotron, where radiation is emitted from the bends in the particle path, or the free electron laser with a periodically-poled magnet called \textit{wiggler}. 
...
Much smaller sources of radiation up to the THz range are the microwave vacuum tubes: \textit{gyrotron}, backward wave oscillators (BWO, also known as \textit{carcinotrons}) and \textit{klystron}.
