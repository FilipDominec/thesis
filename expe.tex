...
\begin{equation} 
\begin{split}
A^{(in 1)}(f)  := E_{x}^{(1)}(f) + \eta H_{y}^{(1)}(f), \quad \quad \quad & A^{(out 1)}(f) := E_{x}^{(1)}(f) - \eta H_{y}^{(1)}(f)\\
A^{(out 2)}(f) := E_{x}^{(2)}(f) + \eta H_{y}^{(2)}(f), \quad \quad \quad & A^{(in 2)}(f)  := E_{x}^{(2)}(f) - \eta H_{y}^{(2)}(f). 
\end{split}
\label{eq_}\end{equation}
The separation result is plotted in Fig. \ref{fg_ampli}, and it allows for crude verification of the computation validity: The incident wave $A^{(in 1)}(f)$ should have smooth spectrum, as it is directly generated by the broadband source. The reflected $A^{(out 1)}(f)$ and transmitted $A^{(out 2)}(f)$ waves should be somewhat complementary, with squares of their amplitude summing up to the square of the incident wave (or less in case of losses). The fourth wave $A^{(in 2)}(f)$ should be negligible, as all wave energy is expected to be absorbed by the PML at the $z$-faces of the simulation volume. In practice it is nonzero due to numerical imprecision and remaining near-field of the structure. In Fig. \ref{fg_separating_wave}, this error 
...
Finally one can easily compute the complex scattering parameters $r(f)$ and $t(f)$ as the ratios of the reflected wave amplitude and transmitted wave to the incident wave, respectively:
\begin{equation} 
\begin{split}
