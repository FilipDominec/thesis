...
The basic principle of the measurement was shared with the scattering-parameter retrieval in FDTD simulations. % TODO add ref
...
\paragraph{Fabrication and sieving of dielectric sphere samples}
...
Triple sieving on commercial sieves, weaved from stainless steel wire, did not provide narrow enough size distribution. We therefore developed a more exact method for sample refinement and characterisation. 
...
Employing the idea of anisotropic sieves from Fig. \ref{fg_double_sieving}a, the author assembled the first prototype based on nylon sieves, as depicted in Fig. \ref{fg_sieving1}. Two glass containers were lathed of a 
% diam
...
\end{figure}
The partially promising results and, more importantly, obvious deficiencies of the previous apparatus motivated the author to build a second one depicted in Fig. \ref{fg_sieving2}.
...
%\paragraph{}
%{{{
Accurate characterisation of the particle statistics was necessary not only to assess the efficacy of sieving, but also to explain possible difference the measured terahertz spectra from the computed ones on a quantitative basis. To obtain reliable statistics, thousands of particles needed to be evaluated, repeatedly for every sample. This, along with the small dimensions of particles, indicated that the optical microphotography with computer image processing automatically resolving each particle would be the most viable way to do so.
...
The first operation on the somewhat blurred photographs [see Fig. \ref{fg_sievingstats}(a)] was to establish the medium value between the image's bright and dark areas, and apply a threshold so that the image was a binary function. Then the \textit{ImageJ}'s \textit{watershed} algorithm was applied to relatively reliably resolve two or more touching particles, and a particle outline was found [cyan lines in \ref{fg_sievingstats}(b)]. Finally, the particle outlines were fitted by an ellipse. 
\begin{figure}[ht] \caption{\textbf{(a)} A small section of a microphotograph of a sample before sieving with a \textbf{(b)} the corresponding identification of particles in ImageJ} \label{fg_sievingstats} \centering 
...
\end{figure}
We accompanied the built-in granulometric algorithm of \textit{ImageJ} with a script enabling to process multiple photographs in a batch, and to plot the resulting histograms of the particle size distribution, which has also been published online \cite{dominec2014_imagej}. Prior to this step, the microphotograph scale was determined from a separate photograph of a ruler.
...
ellipsoidal particles were randomly sprinkled on the glass, they would lie on their flat side -- i.e. their orientation would enable to measure the \textit{medium} and \textit{major} axes of the ellipsoid. This is in contrast with the process of sieving, as described above, where the particles were discriminated according to their \textit{medium} and \textit{minor} axes. 
...
\todo{something should be added??}
%}}}


\paragraph{Cutting of dielectric bars}%{{{ ===========================TODO
...
\end{figure}
%}}}
\section{Laser micro-machining of sieves and fishnet metamaterial layers}%{{{
