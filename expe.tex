...
\paragraph{Thermal emission in THz range}
The blackbody radiation is governed by the Planck law % TODO cite
%$$ ,$$ TODO
from which follows that the luminosity in the terahertz range is always very small, about 300 W sr$^{-1}$ m$^{-2}$ at the room temperature.
Furthermore, all Planck oscillators in the THz range are fully saturated already at the room temperature:
%  although the thermal energy at room temperature $T$ is significantly higher than the photon energy $hf$ at 1 THz
$$k_B T \approx 1.38\cdot 10^{-26} \text{ J K}^{-1} \cdot 300 \text{ K~} \approx 25.8$ meV $\gg h f \approx 4.13$  meV),$$
and therefore the power radiated in the THz range can not be significantly improved by increasing temperature. Particularly, it can be shown that in this part of spectrum it scales only linearly with the temperature $T$; whereas the total power scales as $T^{4}$ (Stefan-Bolzmann law). Sources other than thermal have to be used.
...
\subsection{Terahertz detectors and time-domain spectroscopy}
