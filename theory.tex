...
The group velocity usually points in the same direction as the Poynting vector, but this has not necessarily to be always true. A typical counterexample can be found near resonances in lossy media, including the local ones. Such a behaviour can be traced back to Fig. \ref{fg_oscillator_spectrum}, where the permittivity drops from high values to negative ones. Nonzero losses ensure also the curve of wavenumber $k$ is continuous and smooth, and as a result the magnitude of the group velocity $v_g = \mathrm{d}\omega / \mathrm{d}k < 0$, which is also known as \textit{anomalous dispersion}. This is observed in narrow parts of the spectrum only, around the resonance frequencies where the media have high losses.

Thus, the negative group velocity can also become opposite to the propagation of the light beam, which manifests as negative shift of the light envelope compared to the absence of the medium. Note this can occur in the region of negative phase velocity \cite{dolling2006simultaneous}. %% or superluminal, but only in lossy media and under strong GVD\\
