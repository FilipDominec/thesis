...
The historical review of metamaterials and photonic crystals in this thesis is viewed as three independent paradigms that developed in parallel, and only later they were unified. 
Although each of them, on its own, might have seemed to already have given their fruits, it is their \textit{unification} what renewed the scientific interest and pushed the research further. Useful resources on the history of the field are, among others, in \cite{shamonina2007metamaterials,eleftheriades2012transforming} and at the web pages of Moroz \cite{moroz}.
...
Embedding one different layer, a \textit{defect}, into the middle of a dielectric mirror can introduce a narrow transmission window in the photonic band-gap. The corresponding \textit{defect mode} is characteristic by a strong field concentration in the defect, enabling to construct narrow-band photonic filters that are finely tuned by the defect properties \cite{nemec2005highly}.
...
Conceptual design of a plane-parallel lens: Silin in 1978 \cite{silin1978possibility} %% TODO does not discuss amplification of evanescent waves?
\todo{note:silver lens}
}
...
The design of inhomogeneous structures that could be treated as homogeneous has also a surprisingly long history: Subwavelength wire arrays have been used at least since 1898 as microwave polarisers. From the same time, also the concept of assembling a microwave polarisation rotator from organic or metallic helices is known \cite{bose1898rotation, emerson}.
Inspired by the Lorentz-Lorenz formula, Lord Rayleigh theoretically elaborated the wave propagation through a rectangular lattice of cylindrical or spherical particles in 1892. He also noted the analogy between electromagnetic and acoustic waves \cite[p. 498]{rayleigh1892}. His thoughtful analysis was however still focused to the \textit{static limit} of frequencies negligible to any resonance frequency of the particle. 

Similar quasi-periodic structures, then denoted as \textit{artificial dielectrics} \cite{brown1955properties}, still operated in the static limit below any resonance, found their use in the 1940s to engineer the broad-band permittivity tensor with the application in lightweight lenses for microwave frequencies \cite{kock1948metallic}. Usually the particles consisted of hollow metal waveguides or spheres/disks which increased or reduced the wave phase velocity, respectively. 


%% TODO bruggemann and maxwell-garnett theories for composites; note that composites may be somewhat random if in quasi-static case

%}}}
\subsection{First unification: metamaterials with uncommon parameters} %{{{
1950s brought a first step towards achieving negative effective parameters from man-made periodic structures: The geometry of the wire array was chosen to put the fundamental resonance of the electric dipole \textit{below} the frequency of operation \cite{rotman1962plasma}, and thus acted as Lorentz oscillator with permittivity curve very similar to that in Fig. \ref{fg_oscillator_spectrum}. 
e{boardman2005negative
In the approximation of local media, the \textit{effective permittivity} $\eeff(\omega)$ of this type of artificial dielectrics had a frequency range of values negative, or at least lower than one, leading to the occasional use of the term \textit{artificial plasma} \cite{merkel1973simulation}. 
...
\add{PK-"The word “metamaterial” (MM) appeared first in the paper by Smith et al. in 2000 [6] where a structured material" -- is it true? 
Although somewhat redundant to \textit{artificial dielectrics}, it can be viewed as more fortunate as 
It gained on significant popularity around the millenium break, probably in connection with the Pendry's paper \cite{pendry1999magnetism} which outlined how a negative index of refraction can be obtained in a periodic medium, when metallic wires introducing $\eeff<0$ are interlaced with small loops, known as split-ring resonators (SRR), that introduce $\meff<0$.
}
...
The apparent unification of \textit{metamaterials} with \textit{negative index of refraction} should not be perceived as their sole use -- in fact, even some of nowaday's metamaterial applications are based on purely positive index of refraction.
...
Staying in the electrodynamics field, metamaterial research is not limited to spatial structures only; so called \textit{metasurfaces} are developed to engineer the surface impedance, and to construct absorbers or high-contrast filters with tunable properties.
