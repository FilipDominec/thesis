...
Despite the literature is not unanimous in this, a search shows that the term \textit{metamaterial} was first used by Walser in autumn 1999. Although somewhat redundant to \textit{artificial dielectrics} which has was used for decades before, it can be viewed as more fortunate as it does not imply that the effective parameters must resemble a dielectric, nor require the man-made origin of the structure.
...
Its realization for the microwaves followed suite within a year \cite{smith2000composite}. %% TODO check XXX XXX
