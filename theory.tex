...
If the periodic structure is not homogeneous, that is, $\epsrl(\omega,\rr)$ or $\murl(\omega,\rr)$ depends on $\rr$, the mode functions $\mathbf{u_{em}}(\rr)$ are no more harmonic as they were in Eq. (\ref{eq_blochvac2}), however they still share the periodicity of the lattice as dictated by the Bloch theorem in Eq. (\ref{eq_bloch}). Their exact shape can thus be decomposed into Fourier series
% with the coefficients $c_{ijk}$ being TODO? add equation?
characteristic for the structure, defining possibly nonzero amplitudes for every branch plotted in Fig. \ref{fg_phcfolding}. Note this does not alleviate the ambiguity of choice of the Bloch wavevector $\KK$.

The dependence of the Bloch wavevector $\KK$ on frequency can be plotted in the same way as it was for homogeneous media, with the difference that all branches are present simultaneously. %% TODO note that this fig phcfolding is only for one direction; plotting K~for other directions requires using IFC
