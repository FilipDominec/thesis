...
They could be observed for a local dielectric (Figs. \ref{fg_dcsimpleel} and \ref{fg_dcll}), for nonlocal dielectrics (Figs. \ref{fg_dcll_nl}, \ref{fg_dcllactivity}), as well as for periodic structures in Fig. \ref{fg_phcfolding}. Such regions are denoted as (electromagnetic) \textit{band-gaps}. Their occurence is obviously a very common phenomenon both in dispersive homogeneous media, and periodic structures. 
...
The band-gap behaviour is simplest to describe in isotropic media, analogous to the one-dimensional case: for a frequency within the band-gap, no wavevector $\KK$ can be found under which the electromagnetic wave would propagate. The more complicated cases of a harmonic plane wave in anisotropic homogeneous media, or of the Bloch wave in any periodic structure, the band-gap properties depend on the field polarisation and on the direction under which the wave propagates. Many such media, for example, exhibit a band-gap for one polarisation and only in some set of wave vector orientations.

A familiar scenario is that a wave propagates through a transparent medium (e.g. glass) and impinges an interface with a band-gap medium (e.g. metal). In the latter, it is not allowed to propagate, and all its energy reflects back into the first medium. The Maxwell equations require the fields do not end abruptly at the interface; in particular, it follows from the linearity of the medium that the \textit{evanescent} wave amplitude decays in the second medium in an exponential way with the distance below the interface.
...
% -> Energy concerns: note about how Fourier transform allows no point source to radiate energy within the bandgap
...
One common class of band-gaps is based on \textit{individual resonances} in the medium or structure. These band-gaps are observed in homogeneous media, where the resonances arise from vibration of electrons, atoms or molecules as described above. It is important that these particles are smaller than the  %% TODO ... Bloch? Or what wavelength??
wavelength; their interaction with the wave can be approximated as a point dipole (or possibly quadrupole etc.).

Another important characteristic is that when frequency is changed between the lower and upper limit of the band-gap, some internal change of the response happens. Typically, at the lower band-gap edge, the dipoles are \textit{in-phase} with the wave, whereas at the upper edge, they are \textit{in counter-phase}. 

Periodic structures can similarly exhibit a resonance confined in the unit cell. \todo{++}
