...
For a wave propagating parallel to any axis of a square or cubic lattice with unit cell size $a$, the rule for the Bragg band gaps to be observed is particularly simple:
...
By simultaneous reversal the sign of both $\epsrl$ and $\murl$, the electric and magnetic induction change their sign to be antiparallel to the fields and the wavevector $\KK$ is reversed, associating the triplet with the left hand. 

In the author's view, both terms can therefore be associated exclusively with a proper subset of the \textit{negative-index} media, namely to those where the local parameters of $\eeff$ and $\meff$ exist.
}
\end{enumerate}
%% TODO accumulate literature
