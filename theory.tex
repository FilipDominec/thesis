...
Intuitively it might appear that the requirement of exact equality in the expression (\ref{eq_braggbg}) should cause the Bragg band-gap to have zero width, but in fact the dispersion curves are bent in such a way that the band-gap covers a range of frequencies as in Figs. \ref{fg_phcfolding}a and \ref{fg_phcfolding}d.
...
Moreover, author perceives that the field covered by this thesis is also somewhat burdened by confusion of the terminology: objects denoted simply as \textit{periodic structures} in this text have been denoted in the literature also as \textit{metamaterials}, \textit{photonic crystals}, \textit{photonic materials} or \textit{electromagnetic composites}, \textit{artificial dielectrics} etc. Typically, these terms are used without a proper definition. The issue of terminology is apparently not only a formal one, as it psychologically divides the scientific community and more or less directs the future research.

Physics, and the fields of photonics in particular, have developed rapidly in the recent decades. It may be useful to note that some of the, seemingly novel, concepts were in fact conceived much longer ago, and that sometimes the different terminology leads to re-invention of already known concepts. 

The following humble review of the history of metamaterials and photonic crystals is divided into three paradigms that developed independently, and were unified only much later. 
Although each of the three topics on its own might have seemed to already have given their fruits, it is their \textit{unification} what renewed the scientific interest and pushed the research further.

\subsection{Historical notes} 
\paragraph{Photonic band-gap structures} 
X-ray scattering on crystals was observed by Max von Laue \cite{friedrich1913interferenzerscheinungen} and explained by W. H. and W. L. Bragg in 1910s \cite{bragg1913reflection} as a narrow band-gap resulting from constructive interference of the X-rays reflected from the atomic layers. The very same principle was later %% when?
employed for manufacturing of the \textit{dielectric mirrors}. Operating usually in the optical range, the mirrors consist of periodically alternating layers of quarter-wavelength thickness, made of two dielectrics with different wave impedance.  They are widely used, owing to their advantages over classical metallic mirrors such as higher reflectivity and spectral selectivity.
% related to the \textit{structural colours} found in nature -- who and when did it explain?


\paragraph{Homogeneous media with uncommon parameters} 
Theoretical investigation on electrodynamics of media posessing negative index of refraction, permittivity or permeability can be traced back \cite{agranovich2006spatial} to early discussions of anomalous dispersion near resonances by Lamb or in a 1904 book from Schuster \cite{schuster1904introduction}. L. I. Mandelshtam's lectures on optics touched predicted negative refraction of light in 1940s \cite{mandelstam1971lectures} and were further complemented in 1950s by Sivukhin's discussion of energy concerns \cite{sivukhin1957energy} and Pafomov's notes about reversed \v{C}erenkov radiation \cite{pafomov1956cerenkov}. The Veselago's review of the negative-parameter media \cite{veselago1968} from 1968 later received significantly wider attention.
Note that these papers were developed as the electrodynamics of \textit{homogeneous} media, without assuming microscopic periodicity nor any other way how such a medium should be obtained. 

phc ??
term first used in

\paragraph{Homogenized periodic structures} 
The macroscopic properties of quasi-periodic structures received due attention, too. 
can be identified as a third independent paradigm. 	%	 the macroscopic properties of quasi-periodic structures received due attention, too. the macroscopic properties of quasi-periodic structures received due attention, too. 
...
These periodic structures, denoted as \textit{artificial dielectrics} \cite{brown1955properties}, which were already designed in the 1940s for microwave frequencies -- 
A typical design was random array of short wires that interacted by their electric dipoles with the wave \cite{rotman1962plasma}, and thus acted as Lorentz oscillator with permittivity curve very similar to that in Fig. \ref{fg_oscillator_spectrum} \cite{merkel1973simulation}. % TODO find some older publication?
The unit cell size was kept much smaller than the wavelength, so the task of \textit{homogenization} appeared as settled and apparently did not cause much concerns.





\paragraph{Homogenized periodic structures with uncommon parameters} 
Soon it was realized that artificial dielectrics can attain effective refractive index lower than vacuum \cite{brown1953artificial}, however it took about half a century until the concept of periodic structures was unified with the concept of negative effective parameters.
...
\paragraph{Periodic structures with uncommon parameters and band-gaps} 
...
\add{
It was first studied by Lord Rayleigh in 1887 \cite{johnson2003introduction} in a laminar crystal composed of thin periodic layers of slightly different properties. The field at the lower  whenever there are two different solutions at the same wavevector. % They are typical by 
...
%  Electromagnetic wave propagation in periodic media was first studied by Lord Rayleigh in 1887, in connection with the peculiar reflective properties of a
%  crystalline mineral with periodic “twinning” planes (across which the dielectric tensor undergoes a mirror flip). These correspond to one-dimensional photonic
%  crystals, and he identified the fact that they have a narrow band gap prohibiting light propagation through the planes. This band gap is angle-dependent
