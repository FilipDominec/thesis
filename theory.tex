...
Despite the literature is not unanimous in this, a search shows that the term \textit{metamaterial} was first used by Walser in 1999. Although apparently redundant to \textit{artificial dielectrics}, which was used for decades before,  the term of \textit{metamaterial} can be viewed as more fortunate as it does not depend on whether the structure is man-made or not, nor whether its effective parameters mimic a dielectric or, e.g., a metal.

The research of artificial dielectrics, or newly, metamaterials, gained on significant popularity in the early 2000s, probably in connection with the papers by Pendry \cite{pendry1999magnetism} and by Smith \cite{smith2000composite}. The former outlined how a negative index of refraction can be obtained in a structure composed of metallic wires introducing $\eeff<0$ and of small metallic loops, known as \textit{split-ring resonators} (SRR), introducing $\meff<0$. 
Its realization for the microwaves followed suite within a year \cite{smith2000composite}. 
...
and it may be favorable to substitute metals with certain types of oxides with metallic-like behavoiul in the optical range \cite{naik2011oxides}.
...
\paragraph{Metamaterial applications without unusual parameters} %{{{ ============== (todo expand?)
...
Another example of recent \cite{zhao2009repulsive}, but somewhat controversial \cite{silveirinha2010comment}, possible metamaterial application which does not require to achieve negative $\Neff$, $\eeff$ nor $\meff$ is in reversing the Casimir force.
...
The historical section above suggests the primary difference between the notion of photonic crystals (PhC) and metamaterials (MM) is historical -- that is, each of these notions is connected to a different paradigm, which reflects in the way of description and in the search for application.
