...
\begin{figure}[h!] \caption{Dispersion curves for three different examples of local mediua, comparing the symmetric and Landau-Lifshitz representation of constitutive parameters.\\
...
Both dispersion curves and isofrequency contours predicted by the classical and Landau-Lifshitz representations of the constitutive parameters should be identical. This is illustrated by Fig. \ref{fg_dcll} on three examples: of a medium with an electric resonance, with a magnetic resonance and with both resonances overlapping. The left column shows the \textit{local} permittivity $\epsrl(\omega)$ and permeability $\murl(\omega)$.
...
The previous sections discussed homogeneous media, with the only exception when a refraction at an interface of two media was discussed, which could be easily interpreted on geometrical basis. There is a broader class of structures for which analytical or semianalytical methods have been developed to compute their interaction with electromagnetic waves, such as scattering on dielectric/metallic spheres, diffraction on thin apertures, resonances in orthogonal, cylindrical or spherical cavities, or wave guiding in high-symmetry waveguides or optical fibers. 

The interaction of electromagnetic waves with the rest of possible structures is generally too complex to be expressed analytically, and can only be accessed by numerical methods, some of which are described in the Chapter \ref{chapter_numerical}. However, when these elementary structures are arranged into an infinite array, the resulting structure behaves in a way that is characteristic for periodic structures and its most important traits can again be partially understood on analytical basis.
...
Under the notion of \textit{periodicity} we understand there are discrete translational symmetries in the medium, that is, 
\begin{equation} \epsrl(\rr) = \epsrl(\rr + m_1 \brho_1 + m_2 \brho_2 + m_3 \brho_3) \quad\text{(in three dimensions).} \label{eq_trsym}\end{equation}
where $m_1, m_2, m_3 \in \mathbb{Z}$ and $\brho_1, \brho_2, \brho_3$ are three linearly independent vectors. The same holds for the permeability $\murl(\rr)$.

The points generated by all combinations of possible translations by $m_1 \brho_1 + m_2 \brho_2 + m_3 \brho_3$ form a periodic lattice.
In three dimensions, there are six \textit{crystal families} that differ by their rotation or mirror symmetries of their lattices (i.e. \textit{cubic}, \textit{tetragonal}, \textit{ortorhombic}, \textit{monoclinic}, \textit{hexagonal/trigonal} and \textit{triclinic}).
Combined with all possibilities for the symmetry of the dielectric shape in each unit cell, the periodic structures can be classified into total 230 \textit{crystallographic point groups} in three dimensions. Numerous crystal optics textbooks (e.g. \cite[p. 678]{born1999book}) give more rigorous definitions based on the group theory. 
%fourteen Bravais lattices that differ by their rotation or mirror symmetries. % (i.e. \textit{cubic} P+I+F, \textit{tetragonal} P+I, \textit{monoclinic} P+C, \textit{hexagonal} P, \textit{ortorhombic} P+I+F+C, \textit{trigonal} P and \textit{triclinic} P). 

The minimum volume associated to each point of the lattice is denoted as a \textit{unit cell}. Obviously, the permittivity or permeability in periodic structures needs only to be specified in the domain of one unit cell.

Unless stated otherwise, we will assume the cubic lattice is used, which has highest possible symmetry, as in the cubic lattice, $\brho_1, \brho_2, \brho_3$ are of the same magnitude and mutually orthogonal. Note however that no structure with a periodic modulation of permittivity is truly isotropic, i.e. posessing continuous rotational symmetry.
...
\begin{equation} \E(t, \rr) = \mathbf{u}(\rr)\,\mathrm{e}^{\ii\omega t - \ii\KK\cdot\rr}, \text{ where } \mathbf{u}(\rr) = \mathbf{u}(\rr + m_1 \brho_1 + m_2 \brho_2 + m_3 \brho_3).
...
The field shape given by Eq. (\ref{eq_})
 }
