...
\begin{equation} \mu_0 \HHsd = \label{eq_}\end{equation}
...
Thus, the group velocity $\vg$ can also point opposite to the Poynting vector that represents propagation of the light beam energy, $\mathbf{S}$. 
% This fact can not be easily deduced from the dispersion curves or IFCs. %% TODO  Or can it?
Assuming no energy enters from the second medium, it is obviously the Poynting vector that has to maintain its perpendicular component. 
...
\cite{mikki2009electromagnetic}, group velocity can be negative or superluminal - no problem here?
...
Thus, $\mathbf{u}(\rr)$, $\mathbf{v}(\rr)$ have the same periodicity as the structure, and will be denoted as the \textit{mode functions}. They are generally complex vector functions, so they not only alter the direction and magnitude of the electric and magnetic fields, but can also introduce a \textit{phase modulation} of the wave in each unit cell. 
...
Note this theorem does not determine the shape of $\mathbf{u}(\rr)$, $\mathbf{v}(\rr)$, nor the direction and magnitude of $\KK$, it only states a solution in the form of Eqs. (\ref{eq_bloch}, \ref{eq_blochh}) can be found.
...
This theorem is essential for understanding the electromagnetic behaviour of periodic structures, and it deserves a proof. Originally, it was developed for the electron wave function in crystals on the basis of quantum mechanics. The outline of the proof which is the most often found in textbooks (e.g. \cite[p. 134]{ashcroft2005solid}) is based on that the Hamiltonian operator commutes with the operator of discrete translation by the inter-atomic distance. 
...
The following proof will be modified to use the explicit, classical formulation of electromagnetism as the rest of this thesis does. 
...
the $\Gamma$ point in the Brillouin zone, where $k = 0$, can be identified with Neumann boundary conditions, where we require $dE/dx = 0$ (otherwise the derivative of the field $E$ would not be continuous). Analogically, the $X$ point


			 (Neumann ...  ... ""  vs.  Dirichlet ... M-point ... "E=0")
