...
If the wave is temporally modulated, its amplitude in time is given as a complex envelope function $\E_0(t)$ (instead of a constant vector $\E_0$ in Eq. \ref{eq_pw}). This can be equivalently expressed in the frequency domain as the wave being composed of multiple frequency components, with respective amplitudes given by the Fourier transform of $\E_0(t)$. 
The envelope results from the interference of these components, and therefore its position is determined by their \textit{mutual phase difference}, not by the absolute phase. If the wave propagates through vacuum or other nondispersive medium, each frequency component of the wave acquires phase proportional to its frequency.
