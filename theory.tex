...
In the dispersion equation of local media, Eq. (\ref{eq_dispdet}), the wave vector could be found by solving a set of linear equations. In the particular case of \textit{isotropic} local media, the computation has an even simpler form of an explicit computation of one square root, see Eq. (\ref{eq_dispeq_loc}).
...
\begin{figure}[h!] \caption{Dispersion curves for three different examples of local media, comparing the local and Landau-Lifshitz representation of constitutive parameters. Three rows show media with an resonance in permittivity, in permeability, and simultaneously in both of them.\\
...
The dispersion curves predicted by the classical and Landau-Lifshitz representations of the constitutive parameters should be identical. This is illustrated by Fig. \ref{fg_dcll} on three examples: of a medium with an electric resonance, with a magnetic resonance and with both resonances overlapping. The left column shows the \textit{local} permittivity $\epsrl(\omega)$ and permeability $\murl(\omega)$.

The right column additionally shows the Landau-Lifshitz permittivity $\epsLL(\omega, k)$ as a colour map with blue tone for negative values and red for positive ones. The thin black line overlay connects all points where the dispersion equation (\ref{eq_dispdetLL}) was found to holds by a numerical search.
...
\todo{Why magnetic resonances do not contribute to static permeability?}

\paragraph{The dispersion of nonlocal media} %{{{ 
Further terms in the permittivity $\epsLL(\omega,\kk)$ expansion in Eq. \ref{eq_epsmusd3} lead to the dispersion curves bending.
As a result, the dispersion equations allow multiple solutions with different wavenumber $\kk$ at one frequency $\omega$. (In contrast, the existence of multiple solutions with different $\omega$ for a given wavevector $\kk$ is an usual consequence of frequency dispersion even in local media.)

%}}}
\paragraph{Multiple waves and additional boundary conditions}   %{{{ TODO
\begin{figure}[t] \caption{Dispersion curves for two nonlocal media. The left and right columns of plots show the same information as in Fig. \ref{fg_dcll}. Additionally, the value of the fourth-order expansion coefficient $\alpha$ was plotted with a thin red line. } \label{fg_dcll_nl} \centering  
...
Nonlocal media allow the dispersion curves to bend ...
\label{fg_dcll_nl}

%}}}
\paragraph{Odd expansion terms and optical activity }   %{{{ TODO
\begin{figure}[t] \caption{Dispersion curves for two nonlocal media. The left and right columns of plots show the same information as in Figs. \ref{fg_dcll} and \label{fg_dcll_nl}.} \label{fg_dcllactivity} \centering  
