...
By \textit{negative} parameters is understood either the negative real value of permittivity $\epsrl(\omega)$, permeability $\murl(\omega)$, 
or negative index of refraction. In transparent media without spatial dispersion, the latter coincides with $\epsrl(\omega)$ and $\murl(\omega)$  being negative simultaneously.  %% elaborate this a bit, using complex multiplication? Explain why negative branch is chosen, but this perhaps should be put above in the discussion of group velocity.
...
It was the Veselago's review of the negative-index homogeneous media \cite{veselago1968} from 1968 that later received significantly wider attention. Veselago used % is he first?
the term of \textit{left-handed media} and speculated to what extent they might exist in the nature, but did not discuss the more general theory of the spatial dispersion, which may lead to negative refraction, too.
...
To the author's knowledge, all demonstrations of s tebou metamaterial sub-wavelength imaging demonstrated so far were therefore either only proof-of-concept, microscopic devices, or were limited to the easier accessible microwave region. % (verify there is no breakthrough in the IR lately?)
...
\paragraph{Metamaterials with strong chirality} %{ {{ ============== (todo expand?)
In spite of the above mentioned application being popular, many metamaterial applications \cite{zheludev2010road} do not involve negative nor close-to-zero effective parameters.
Among these are highly sensitive sensors, light modulators or devices making use of the enhanced nonlinear interaction.

An example of a recent \cite{zhao2009repulsive}, but somewhat controversial \cite{silveirinha2010comment}, metamaterial application which does not require negative values of $\Neff$, $\eeff$ nor $\meff$ is in reversing the Casimir force.

%}}}

\subsection{Second unification: metamaterials with photonic crystals} 
Over the 20th century, the paradigms of metamaterials (MM) and photonic crystals (PhC) apparently developed independent of each other. The situation started to change in 2000s, when the metamaterial structures were realized to operate at near-infrared or optical frequencies. As already stated, reduction of the unit cell size becomes a big issue; not only is the contemporary technology somewhat limited in its resolution, but more importantly, with any choice of available materials the dissipative losses become a problem, particularly when the tight confinement is required. 

For both reasons, in the optical range a compromise is usually made that the particle size becomes only few times smaller than the wavelength of operation, and such a structure then classifies at the boundary of metamaterials and photonic crystals. For example, this is the case of  % TODO 3 examples here!  \cite{yannopapas2005negative} \cite{shamonina2007metamaterials}
%Chakrabarti2001_From_effective_response_to_PBG.pdf
...
% todo write about {\textit{plasmonic nanoparticles} at optical frequencies?


Probably the miniaturisation issues have incited also the purely theoretical studies of the interplay of the localized, metamaterial-like, resonances and distributed, photonic crystal-like, resonances in the unit cells.
% typically one scans the relative cell spacing...

%PhCs with metallic/metamaterial inclusions
%ad_Shi2007-Effects_induced_by_Mie_resonance_in_2D_PhC $ ls -1
%Antonnoyiannakis1997-Mie_resonances_and_bonding_in_PhC.pdf
%Rybin2015-Phase_diagram.pdf
%Shi2007-Effects_induced_by_Mie_resonance_in_2D_PhC.pdf


The full electrodynamic behaviour of the unit cell has also been substituted by effective parameters. This is equivalent to examining a one-dimensional structure of alternating layers of positive and negative index of refraction.
An atypical type of a photonic crystal is formed which exhibits a specific \textit{zero order photonic band-gap} when the phase advance through the layer stack is zero. (Note that typical photonic band gaps occur when the phase advance is $2\pi M, M\in \mathbb{N}$).
...
%Liang2003-Unusual_narrow_bands_in_LHM_stack.pdf
%	
%YihangChen2009-Unusual_bands_of_1D_PhC_containing_SNG.pdf
%
%Ad_Monsoriu2:
%Depine2006-ENG_and_MNG_band_gaps_in_1D_PhC.pdf




\section{The boundary between photonic crystals and metamaterials} 
The historical section above suggests the primary difference between the notions of photonic crystals (PhC) and metamaterials (MM) is historical -- that is, each of these notions is connected to a different paradigm, which reflects in the way of description and in the search for application.

At the end of the previous section it is however shown that the most recent research involves also structures at the boundary between both paradigms, and it is a pertinent question of whether there are some objective apriori differences. Several different criteria are listed below, which are not strictly equivalent. As a result, there exists a "grey zone" of structures that can more or less fit into both categories  simultaneously.
...
\paragraph{Metamaterial definitions}
whatis.com (http://whatis.techtarget.com/definition/metamaterial):
{\quote A metamaterial is a (\ldots) substance whose properties depend on its inter-atomic structure rather than on the composition of atoms themselves.}
...
{\quote MMs are artificially structured materials used to control light, sound and many other physical phenomena. The properties of metamaterials are derived from the inherent properties of their constituent materials, as well as from the geometrical arrangement of these (\ldots)}
