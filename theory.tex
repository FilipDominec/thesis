...
In many papers on periodic structures, all dispersion curves are plotted as \textit{folded} into the $K\in\langle0, \pi/a\rangle$ region only (\cite{obrien2002photonic, chakrabarti2001magnetic}), while some employ the symmetry to fold all curves into the $K\in\langle0, \pi/a\rangle$ (e.g. \cite{yannopapas2005negative}). In some other references, such as \cite{mortensen2010unambiguous} or \cite{yeh1977electromagnetic}, unfolded dispersion curves are used.
...
The unification of mm and phc become apparently a more important topic when the metamaterial structures were realized using the available materials. As already stated, reduction of the unit cell size becomes a big issue if the metamaterial is to operate at near-infrared or optical frequencies. Not only the technology is required to have nanometric accuracy in the whole structure, the choice of materials becomes more stringent to the allowable losses, the more the tighter the field confinement is. 

Therefore, in the optical range a compromise is usually made that the particle size becomes only few times smaller than the wavelength of operation, and such a structure then classifies at the boundary of metamaterials and photonic crystals. For example, this is the case of  % TODO 3 examples here!  \cite{yannopapas2005negative} \cite{shamonina2007metamaterials}
