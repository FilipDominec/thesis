...
which allows to simplify Eq. (\ref{eq_chi_convol_nonloc}) in the way that in local media only the temporal convolution has to be computed. Then the higher order terms including $\gamma_{e,m}$ and $\alpha_{e,m}$ in Eqs. (\ref{eq_epsmusd1}) vanish:
$$ \epsrn(\omega,\kk) = 1 + \chi_e(\omega) = \epsrl(\omega), $$
\begin{equation} \murn(\omega,\kk) = 1 + \chi_m(\omega) = \murl(\omega). \label{eq_epsmusd2}\end{equation}
...
Here, we will show here that the magnetic response can be fully expressed by a certain form of $\epsrn(\omega, \kk)$ dependence on $\kk$. The spatial-dispersive function $\epsrn(\omega, \kk)$ will then consist of
\begin{enumerate}
 ...
 \item{Additionally, the spatial dispersion allows to describe other phenomena that can not be described by the local theory.}
\end{enumerate}
The magnetic permeability becomes a mere constant of the magnetic response of vacuum $\murll(\omega, \kk) = 1$, thus reducing the complexness of the computation. %% [as in Eq. (\ref{eq_ce})]
