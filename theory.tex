...
A related mathematical proof of Kramers-Kronig relations can be derived from the analytic properties of the response function in the complex plane. %% TODO cite
...
\mdf{
in isotropic, local medium the wave propagation velocity depends on permittivity and permeability, which is a function of frequency on its own\\
$$N = sqrt(eps(omega))$$\\

figure dispersion curve - scan over resonator strength\\

note that the shape of dispersion curves can be interpreted also as the polariton branch (horizontal) and photon branch (light-line, growing),\\
which couple and form the anti-crossing shape known e.g. from the theory of coupled oscillators (cite Landau-Lifshitz here)\\

Resonance in magnetic permeability leads to a similar behaviour\\
...
\mdf{
in the presence of resonance, omega is not a simple function of k; as is shown below, conversely, in the presence of nonlocal response, k~is not necessarily a simple function of omega\\
...
The terms of \textit{nonlocality} and of \textit{spatial dispersion} are used interchangeably in the literature. The difference seems to be related to the way one thinks about the medium -- while \textit{nonlocality} is obviously related to the description in the real space (cf. Eq. (\ref{eq_chi_convol_nonloc}), \textit{spatial dispersion} derives from that the response to a harmonic wave is dependent also on the wave vector $\kk$. In the author's view, the term \textit{spatial dispersion} is therefore of slightly narrower meaning, as it implies that a plane wave in infinite medium is considered.
%}}}
