...
In a very similar manner as in the local theory above, we assume a plane wave $\E(t, \rr) := \E_0 \, e^{\ii\omega t - \ii\kk\cdot\rr}$ propagates through the medium, without loss of generality thanks to any wave being a superposition of plane waves.
...
Assuming the dependence of the medium permittivity $\varepsilon(\omega,\kk)$ or permeability $\mu(\omega,\kk)$ vary only slowly with $\kk$, we generally express them as power series \cite[p. 367]{landau1984electrodynamics}:
\begin{equation} \varepsilon(\omega,\kk) = \varepsilon(\omega) + \ii \gamma_{\text{E}}(\omega) \kk + [\alpha_{\text{E}}(\omega) \kk] \kk + \ldots, \label{eq_epssd1}\end{equation}
\begin{equation} \mu(\omega,\kk)		 =		   \mu(\omega) + \ii \gamma_{\text{M}}(\omega) \kk + [\alpha_{\text{M}}(\omega) \kk] \kk + \ldots, \label{eq_musd1}\end{equation}
where $\gamma_{\text{E,M}}(\omega)$ are tensors of the third rank and $\alpha_{\text{E,M}}(\omega)$ are tensors of the fourth rank. After the matrix multiplication with $\kk$, they all yield rank-2 tensors that add up to the possibly anisotropic permittivity.
...
which allows to simplify Eq. (\ref{eq_chi_convol_nonloc}) in the way that in local media only the temporal convolution has to be computed. Then the higher order tensors $\gamma_{\text{E,M}}, \alpha_{\text{E,M}}$ in Eqs. (\ref{eq_epssd1}, \ref{eq_musd1}) are identically zero:
\begin{equation} \varepsilon(\omega,\kk) = \varepsilon(\omega), \label{eq_epssd2}\end{equation}
\begin{equation} \mu(\omega,\kk)		 =		   \mu(\omega). \label{eq_musd2}\end{equation}
