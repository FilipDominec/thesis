...
Probably it were the miniaturisation issues that incited the studies of the interplay between the individual and Bragg-type resonances, the former being typical for metamaterials, the latter for photonic crystals. Typically, one either scans the relative cell spacing (this can be done in simulations \cite{shi2007} as well as in experiments \cite{rybin2015}),  % and \cite{antonoyiannakis1997mie}?
...
In the more abstract studies, the structure of the unit cell, with its full electrodynamic behaviour, is substituted by a flat layer characterized only by its effective parameters which may be negative at certain frequencies. % This is equivalent to examining a one-dimensional structure of alternating layers of positive and negative effective parameters.
...
The section above suggests the primary difference between the notions of photonic crystals (PhC) and metamaterials (MM) is historical -- that is, each of these notions is connected to a different paradigm, which reflects in the way of description and in the search for application.

The reason behind merging of the MM and PhC paradigms are the practical aspects that lie at the boundary, and it is a pertinent question of whether there are any objective differences between the notions of MMs and PhCs at all. 

Several different criteria are listed below, which are not strictly equivalent. As a result, there exists a "grey zone" of structures that can more or less fit into both categories  simultaneously.

\paragraph{Difference in the Bloch wavelength} %{ {{ ==================================================
Both the metamaterial (MM) and photonic crystal (PhC) concepts are inspired by solid-state physics, namely by theories of waves propagating in solids. The fundamental difference between these is \textit{which type of wave} was the initial analogy: 
\begin{enumerate}
 \item{Metamaterials are related to \textit{optical waves} propagating in the quasi-homogeneous crystal lattice, whereas } 
 \item{the theory of photonic crystals was developed on the basis of \textit{electron waves} in the crystal lattice.} 
\end{enumerate}
The wavelength of the valence-band electrons wavefunction is similar to the inter-atomic distance ($\approx 10^{-10}$ m), demanding the description through the Bloch theorem, whereas the wavelength of light is roughly four orders of magnitude larger.

Some papers base their definition of metamaterials on the wavelength criterion. For instance, ref. \cite{tsukerman2011nonlocal} defines that \textit{metamaterials (are) artificial periodic structures with features smaller than the vacuum wavelength}. 

Such definitions however lack rigour: the ratio of the (Bloch) wavelength to the unit cell size tends to be between 0.1 and 1 in all periodic structures at their expected frequency of operation.
The boundary between the realizable MM and PhC fields is therefore much less clear, and this is the reason why the more general term of \textit{periodic structure} is predominantly used in this thesis.  % TODO rewrite
...
At a given frequency of operation, any periodic structure can either allow the Bloch wave to propagate, or exhibits a band-ga. The photonic bands are thus well separated by the band-gaps (PBGs), and the number of the photonic band, counted from zero frequency, gives an important characteristic of the structure.
...
Finally, we return to denoting of MMs and PhCs as two \textit{paradigms} rather than being two rigorously defined classes of  structures. Throughout their development, they have been associated with different purposes: the development of PhCs focused on the band-gap phenomenon, and aimed to control \textit{whether} the electromagnetic waves can propagate or not. 

With MMs, the aims were more ambitious -- their analysis and synthesis were always concerned \textit{how} the waves propagate. Such a MM description aimed towards describing the medium in terms of \textit{effective parameters}

 and different analogies.
