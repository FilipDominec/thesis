...
\begin{equation} 
\left.  \begin{array}{cc}
\mathbf{u_{e}}(\rr) := \E_0 e^{2\pi\ii (m_1/a_1 + m_2/a_2 + m_3/a_3)} \\ %% TODO use mathbf here? or is this whole a rubbish?
...
The notation of high-symmetry points in the  %% TODO 

%}}}
%% TODO add an example plot of vacuum and comment
%% \paragraph{Dispersion curves for the reduced Brillouin zone} %{{{ 
%% Gamma-X-K-Gamma plot
%% %}}}
\paragraph{Tight-binding model for photonic band-gaps} %{{{
The notation of the high-symmetry points in the Brillouin zone is a good background to take  a different view to the origin of the dispersion, which can be drawn in analogy with the \textit{tight-binding},   %% TODO cite?
...
In particular, the $\Gamma$ point in the Brillouin zone, where $k = 0$ in any dimension, can be identified with the \textit{Neumann boundary conditions} $dE/dx = 0$. This condition is equivalent to the field being stationary on the boundary, since  otherwise, the derivative of the field $E$ would not be continuous. % this also holds for other boundaries, on the $y$- and $z$-axis
...
%}}}
\paragraph{Spatial dispersion in photonic structures} %{{{  TODO
Near the X, K~or M points, the spatial dispersion must be strong to account for the zero group velocity, and actual bending of the dispersion curve downwards in the adjacent zone. The effect can however not be directly interpreted as the negative group velocity, as the dispersion curve only corresponds to one term in the Fourier expansion of the field shape.  \todo{rewrite + add}
%}}}
\subsection{Photonic band-gaps} %{{{  TODO
...
\paragraph{todo}%{{{
\add{
Nonresonant metamaterials can be identified with those designed to operate in the first photonic band. The reason is given by the facts that every type of resonance is associated to some band gap, and by that the desired effects of a resonance  are obtained at frequencies \textit{above} it, at least in all metamaterials known to the author. Let us stress that the word \textit{nonresonant} is inseparably linked with specifying the relatively low frequencies of operation, since for nearly any structure, some electromagnetic resonances can be found at their respective frequencies.

The useful properties that can be obtained by averaging the properties of the constituent materials \cite{oganisian2014observation} or by engineering the unit cell are typically a strongly anisotropic tensor of permittivity \cite{kadlec2008, sibik2010dp}. Strictly speaking, this class of structures does not require periodicity, 
