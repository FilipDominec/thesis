...
employed for design and manufacturing of the \textit{dielectric mirrors}, which are made by sputtering multiple layers two alternating dielectrics of different wave impedance and with quarter-wavelength thickness. They find a wide variety of applications, owing to their advantages over classical metallic mirrors such as higher reflectivity and spectral selectivity.  %% this is related to the \textit{structural colours} found in nature -- who and when did it explain?
...
%}}}
\paragraph{Band-gap used in fibers and waveguides} %{{{ ============= cites: PCF, DBR,   incl Zengerle
...
The \textit{photonic-crystal fiber} (PCF), which differs from the common index-guiding fibers by that the light is confined in its core by a photonic band-gap in both lateral directions, was conceived in 1970s. Its special capability is to strongly guiede the wave even in a hollow core. % todo cite, where are the 70's ?

The demands for achieving a complete three-dimensional band-gap can be reduced when a planar structure is etched into a dielectric slab. It is also much easier to fabricate using developed techniques such as photolitography. A \textit{planar band-gap waveguide} can be formed by a linear defect in the photonic crystal slab, which restricts the light by the total internal reflection horizontally and by the means of the two-dimensional band-gap vertically. Such structures can form waveguides, beamsplitters and filters in \textit{integrated optics}. 
% Beam propagation in periodic planar waveguides \cite{zengerle1987light}
%% TODO PhC slabs \cite{zengerle1987light}

As a complementary concept, a longitudinal modulation can be inscribed into the index-guiding fibers, forming a \textit{distributed Bragg reflector}. Its filtering capabilities can be made extremely narrowband, which is employed in fiber lasers, telecommunication, sensing, etc. % TODO cite, when first?
...
or negative index of refraction which, in transparent media, coincides with $\epsrl(\omega)$ and $\murl(\omega)$  being negative simultaneously.  %% elaborate this a bit, using complex multiplication? Explain why negative branch is chosen, but this perhaps should be put above in the discussion of group velocity.
...
The same mechanism causing the radio wave reflection from dilute plasma in the Earth's ionosphere was predicted as early as in 1839 by Gauss; its application for over-the-horizon communication was suggested in 1901-1902 by Heaviside and Kennelly, starting its ongoing research.

\textit{Negative permeability} manifests similarly from the electrodynamic point of view, but is observed much less often, due to the absence of magnetic monopoles, above magnetic resonances in matter.
...
It was the Veselago's review of the negative-index homogeneous media \cite{veselago1968} from 1968 that later received significantly wider attention. Veselago coined % is it true ?
the term of \textit{left-handed media} and speculated to which extent media with negative index of refraction due to the simultaneously negative permittivity and permeability might exist in nature (but did not discuss spatial dispersion, which may lead to more general negative refraction, too).
% note about spatial dispersion in homogeneous media? Or in an extra paragraph later, under 2nd unification?
...
a plane-parallel slab with an isotropic negative index of refraction and sufficient thickness would refract waves emanated from a point source towards a new focus in its volume, and similar negative refraction forms a second focus behind the slab. It is thus often referred to as a \textit{negative-index lens}, although unlike in classical lenses, its focal point nor optical axis are not defined.

Clearly, the negative-index slab would be free of spherical aberrations, allowing a wide-angle and high numerical aperture for the imaging. There was no discussion yet of overcoming the diffraction limit nor of amplification of evanescent waves.  % should the thin silver lens be noted here, or elsewhere?

%}}}
\paragraph{Media with parameters near zero}%{  {{ ================= TODO
Media where $\epsrl(\omega)$ or $\murl(\omega)$ have a very low value compared to vacuum are another class uncommon in nature. They are often denoted as \textit{epsilon-near-zero} (ENZ) and \textit{mu-near-zero} (MNZ). % and perhaps note superhigh opt activity?
The result of $\epsrl(\omega)\sim 0$ or $\murl(\omega)\sim 0$ is also the index of refraction being close to zero, $N\sim 0$. 
Although e.g. the physics of the ionosphere involves regions where the permittivity transits from negative to positive values, the scientific interest in this class of media started with the paper of  %% TODO
published after the merging of the paradigm with metamaterials, and therefore such media are often denoted syncretically as \textit{zero-index metamaterials}. This is a somewhat confusing term, as the papers focused on their peculiar macroscopic properties usually do not assume any structuration and are thus also applicable to natural homogeneous media of such properties.

Typical of this class is a very small wavevector, which can be approximated to the case of the whole $N\sim 0$ region oscillating in-phase. Wave impinging from air to such a medium refracts from the normal; for angles higher than the usually small critical angle the wave undergoes the \textit{total external reflection}. 
...
%}}}
\paragraph{Transformation optics and electromagnetic cloaks}%{  {{ ================= TODO
\cite{eleftheriades2012transforming}

\cite{alitalo2009electromagnetic} Electromagnetic cloaking % by homogeneous media ???
was considered in the early 60s by Dollin \cite{dollin1961possibility}
...
\paragraph{Analysis of nonresonant sub-wavelength structures} %{{{ ================== (add Brugg + M-G)
...
% \mdf{Continuous wire arrays in waveguides etc., } ??

%}}}
\paragraph{Negative effective permeability and negative refractive index}  %{ {{ ================== TODO  "MM" term first?
%  - resonant magnetic dipole arrays

\cite{smith2000composite}

\mdf{PK-"The word “metamaterial” (MM) appeared first in the paper by Smith et al. in 2000 [6] where a structured material" -- is it true? 
Although somewhat redundant to \textit{artificial dielectrics}, it can be viewed as more fortunate as 
It gained on significant popularity in the early 2000s, probably in connection with the Pendry's paper \cite{pendry1999magnetism} which outlined how a negative index of refraction can be obtained in a periodic medium, where metallic wires introduce $\eeff<0$ and small metallic loops embedded in the lattice, known as \textit{split-ring resonators} (SRR), introduce $\meff<0$.

\cite{smith2000composite}
}

\textit{split-ring resonator}

A major issue are the dissipative losses in resonant periodic structures, which were not sufficiently resolved yet. Some quantitative improvements can be easily done, namely thickening the metallic structures and increasing their inductance-to-capacitance ratios of the split-ring resonators (SRR) \cite{zhou2008efficient}. At the near-infrared and optical wavelengths, SRRs are better substituted by perforated metallic sheets, so called \textit{fishnets}, % todo cite
and it may be favorable to substitute metals with certain types of oxides with near-infrared resonances \cite{naik2011oxides}.

\paragraph{Superlensing with metamaterials}
...
\paragraph{Metamaterial cloaking}
The apparent unification of \textit{metamaterials} with \textit{negative index of refraction} should not be perceived as their sole use -- in fact, a notable part of contemporary metamaterial research is based on purely positive index of refraction.
%Transformation optics

%% TODO Decide if to put here or earlier!
The most important example is the \textit{electromagnetic cloak}, a structure causing and obstacle become invisible by guiding the wave coming from any direction around its hollow core so that the obstacle nor the cloak cast any shadow. This is typically achieved the use of $\Neff\in \langle0,1\rangle$ properly deployed by transformation-optics calculations. %TODO move to continuous media
A simplified task is to construct the \textit{carpet cloak} which conceals an obstacle near a mirror so that it is not visible for reflected light. Note, however, that a carpet cloak can also be made of homogeneous, anisotropic dielectrics  \cite{wang2013homogeneous}.
%Yet another group are static magnetic cloaks
All reported cloaks are, however, strongly limited in band-width and moreover, the presence of any macroscopic cloak is revealed by its losses. Obviously a great conceptual breakthrough would be needed to overcome these issues. 

\paragraph{Metamaterial applications without unusual parameters}
In spite of the above mentioned applications, many metamaterial applications do not involve unusual effective parameters \cite{zheludev2010road}.
Among these are  reaching strong chirality, building sensors and light modulators or devices making use of the enhanced nonlinear interaction.

Another example of recent \cite{zhao2009repulsive}, but somewhat controversial \cite{silveirinha2010comment}, metamaterial research which does not aim to achieve negative $\Neff$, $\eeff$ nor $\meff$ is in reversing the Casimir force.

%Metasurfaces.

%}}}
\subsection{Second unification: metamaterials with photonic crystals} %%						 TODO%{{{
The unification between 

\cite{shamonina2007metamaterials}

PhCs with metallic/metamaterial inclusions, [Monsoriu, Lina Shi and friends]

the unification of mm and phc become apparently a more important topic when the practical realizations of the structures were tested
\cite{yannopapas2005negative}

\mdf{\textit{plasmonic nanoparticles} at optical frequencies}
...
\section{The boundary between metamaterials and photonic crystals} 
