...
For simplicity, we assumed here that the relative permeability $\murl = 1$; the extension to other cases is possible, too. %% TODO Do epsilon and mu commute, if they are both tensors??
...
\begin{equation} 
\left.  \begin{array}{c}
\epsLL(\omega,\kk) = 1 + \chi_e(\omega) - \frac{1}{\omega^2 \mu_0}\left(1 - \frac{1}{\murl(\omega)}\right) \kk\times(\kk\times \E),\todo{KxKx-tensor} \medskip \\
\muLL(\omega,\kk) = 1. 
\end{array} \quad \right\} \quad \text{(in local media)}
\end{equation} 
The classical,  symmetric, approach using parameters $\epsrl(\omega,\kk),\murl(\omega,\kk)$, and the Landau-Lifshitz approach of gathering all medium-related effects in the permittivity $\epsLL(\omega,\kk)$ are only two examples of all possible transformations of medium parameters that keep the Maxwell equations unchanged. If needed, it is formally possible to use any other arrangement, which may moreover be frequency dependent, leading to a physically realistic pair of parameters with uncommon properties \cite{skaar2014}.
...
\paragraph{How a local dielectric manifests in the Landau-Lifshitz model}  % TODO%{{{
The dispersion curves and isofrequency contours predicted by the classical and Landau-Lifshitz representation of constitutive parameters should be identical. For a local, isotropic medium, the dispersion has already been shown in Fig. \ref{fg_dcsimpleel}. A very similar figure is shown in \ref{fg_dcll}
The left panel is identical to that shown in Fig. \ref{fg_dcsimpleel}, as an assumption is made here that the medium is fully described by the \textit{local} permittivity $\epsrl(\omega)$ and permeability $\murl(\omega)$.\\
The right panel additionally shows the Landau-Lifshitz permittivity $\epsLL(\omega, k)$ as colour contours. The thin black line overlay connects all points where the dispersion equation holds, $\epsLL(\omega, k) \,\omega^2 / k^2 = 1$. As a confirmation of the equivalence of this Landau-Lifshitz model with the classical one, the original green dispersion curve from Fig. \ref{fg_dcsimpleel} is retained that was computed using the classical approach based on the Eq. (\ref{eq_dispeq_loc}). 

\begin{figure}[th] \caption{Dispersion of a local medium, comparing the symmetric and Landau-Lifshitz representation of constitutive parameters. Left column: Local parameters $$, $$ as functions of frequency. Right column: Dispersion curves computed using Eq. () an electric resonance in the local permittivity, 
} \label{fg_dcll} \centering  
	\includegraphics[width=.6\textwidth]{img/dispersion_landau_lifshitz/dispersion_ll_el.pdf}
%\end{figure}
%\begin{figure}[h] \caption{\todo{Magnetic resonance in L-L view of a local medium}} \label{fg_dcllmag} \centering  %% TODO refto
	\includegraphics[width=.6\textwidth]{img/dispersion_landau_lifshitz/dispersion_ll_mag.pdf}
%\end{figure}
%\begin{figure}[h] \caption{\todo{Overlapping electric and magnetic resonance in L-L view of a local medium}} \label{fg_dcllelmag} \centering  %% TODO refto
	\includegraphics[width=.6\textwidth]{img/dispersion_landau_lifshitz/dispersion_ll_elmag.pdf}
\end{figure}
