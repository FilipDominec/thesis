...
Further, all forward and backward propagating solutions are mirror-symmetrical with respect to the vertical $K=0$ axis, except for structures with optical activity (c.f. Fig. \ref{fg_dcllactivity} or even breaking the time-reversal symmetry (c.f. \cite{vanwolleghem2009unidirectional}). Thus, in many papers on periodic structures \todo{cite some!}
...
\item{
At the first point of folding, the mode function of $\mathbf{u_e}(\rr) := E_0 e^{2\pi\ii/a}$ and the formal solution of the \textit{backward} wave is used with $K := 2\pi/a - \omega/c$. 
} 
\item{
When the dispersion curve touches the $K=0$ axis at the frequency
} 
 \end{enumerate}

In spite what was written here, in the section discussing the numerical methods we will show that there indeed exist nontrivial rules to prefer one particular from all dispersion curves in the unfolded plot. In the Results section we will show there is some physical interpretation of this.
