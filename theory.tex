...
For every single photonic branch, there are two sets of IFC for two different polarisations of the transverse wave, but for simplicity we plot only one polarisation. % \todo{discuss? In uniaxial media, where two diagonal components of the }
...
The component of wave vector parallel to the interface (chosen as $k_x$ here and indicated by the vertical dashed lines) must be maintained upon refraction. This rule can be intuitively deduced from the continuity of phase in both media, as well as from the Noether theorem applied to the infinite translational symmetry of the interface. Transferring the vertical dashed lines to IFC plot of another medium and finding the intersections with an IFC of the corresponding frequency, one can find the new wavevector. 
...
Usually in spectral regions near a resonance, also the quadratic or even higher terms in the Taylor expansion of the $\omega(\kk)$ dependence shall be taken into account. This effect is known as the \textit{group velocity dispersion} as it is equivalent to the group velocity $\vg(\kk)$ being dependent on the wavevector (or, frequency, if reformulated). It results in temporal distortion of the wave envelope $\E_0(t)$.  %% TODO ... and what about distortion in space?
%}}}

\paragraph{Propagation of a beam in anisotropic media}  %{{{  todo near single resonance
The propagation of a beam is illustrated in Fig. \ref{fg_ifc2} with three slightly different wavevectors. For simplicity, a monochromatic wave was used, so brown, black and violet were chosen for the three example wavevectors to prevent confusion with the rainbow-like frequency color map used in Fig. \ref{fg_ifc}. 
...
Fig. \ref{fg_ifc2}c shows again the special case of anisotropic medium, where a wave propagates near its optical axis, and where the Snell law, with the generalized notion of the index of refraction, can still be used to determine the refraction of a plane wave.
However, there is a pitfall if one tries to apply the Snell law for prediction of how the beam will refract. It originates from the differential nature of the group velocity definition in Eq. (\ref{eq_vg}). 
...
\mdf{
TODO now that spatial dispersion is defined, the IFCs can have more exotic shapes than just cone sections 
show that nonlocal/photonic media can lead e.g. to "beam collimation" when ifc goes flat
}

\mdf{
show that near a single resonance, the phase velocity can drop \\
TODO FIGURE?: a) eps resonant, mu constant     b) N positive, but group velocity negative\\
...
show that "signal propagation velocity" is a problematic term, because a finite-support function spans over infinite spectrum, where group velocity has to change and differ \\
}
%}}}
...
%}}}
\paragraph{Energy and the Poynting vector}  % TODO%{{{
\mdf{
