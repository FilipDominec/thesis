...
\label{eq_blochvac}
\end{equation}
However, this is not the only possible representation of a plane wave; the formulation of the Bloch wave in Eqs. (\ref{eq_bloch}, \ref{eq_blochh}) is inherently ambiguous. Any lattice of periodic unit cells may be imagined in vacuum. For illustration, given one cubic lattice with an arbitrary unit cell size $a$, % TODO represent by lattice vectors G?
