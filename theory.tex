...
Noticing that the leftmost term is also a linear transformation of the vector $\E$, expressed by the tensor
$$ - \kk (\kk \cdot \E) \equiv 
...
%	\left(\begin{array}{c} E_x \\ E_y \\ E_z \end{array} \right),
%	$$
the dispersion relation can be reformulated\cite{born1999book} as a requirement for the following determinant to be zero:
\begin{equation} 
	...
	\end{array} \right| = 0,
\label{eq_dispdet}\end{equation}
which ensures a solution of Eq. \ref{eq_wavek} can exist with nonzero $\E$.
The branches of solutions of this equation divide into two groups:
\begin{enumerate}
...
Above $\omega_0$, the dipoles of the Lorentz oscillator can no more follow the electric field and are opposite to the electric field, thus pushing the permittivity below zero. With further increase of the frequency, the permittivity crosses zero at the $\omega_{\text{L}} > \omega_0$, which allows the transverse wave to propagate. Therefore, the \textit{upper polariton branch} starts from $\omega_{\text{L}}$. In case of a Lorentz oscillator unaffected by the presence of others, the difference $\omega_{\text{L}} - \omega_0$ can be computed from the magnitude of the oscillator (using the Lydanne-Sachs-Teller relation).\cite{klingshirn2007semiconductor}
The same behaviour is observed for a single resonance in the permeability $\mu_r(\omega)$, and will be typical also for the spectra of periodic structures described later.
...
%}}}
\paragraph{Longitudinal waves in dispersive media}  % TODO%{{{
The solution of Maxwell equations for a harmonic wave, 
$$ \quad\quad\quad\quad\quad\text - \kk (\kk \cdot \E)      +          k^2 \E            = +\mu_0 \varepsilon_0 \omega^{2} \E, \quad\quad\quad\quad\quad\text{(\ref{eq_me4} again)}$$
also allows the existence of longitudinal waves with electric field parallel to the wave vector $\E || \kk$. If $k>0$, they have nonzero solution only when either $\varepsilon_r(\omega)' = 0$, or $\mu_r(\omega)' = 0$, and therefore in local media their dispersion curve is a horizontal line at $\omega = \omega_{\text{L}}$, independent of $k$.



longitudinal waves - plasma oscillations, have flat dispersion in the approx of local media, which means they would form standing oscillations and not propagate ...
%}}}
...
It is often important to describe the dispersion curves also for different wave angles, which is the best accomplished by plotting the frequency $\omega(\kk)$ as the function of the projection of two components of the \textit{wave vector} $\kk$. 
...
information about the medium interaction with electromagnetic waves, but also allows for geometrical analysis. It is, however, used to plot dispersion in media with low losses only ,  % TODO confusing
and each branch has to be plotted separately to prevent the contours from overlapping.
...
\mdf{
Isofrequency contours (IFC), or also Equifrequency contours (EFC)\\
