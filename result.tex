...
The low-pass filtering capability of this structure is shown in more detail in the continuous scan through $d_x$ in Fig. \ref{fg_yslit_xscan}a. 
The unit cell size in the $x$-direction for both presented figures was $a_x = 100$ $\upmu$m. 
The highest frequency in the plots was determined by the onset of the first diffraction order as $c/a_x = 3.0$ THz. 
...
This phenomenon known as \textit{extraordinary transmission} is mediated by electromagnetic waves propagating along the metallic surface (i.e. parallel to the $x-$ axis. The field distribution is similar to surface plasmons-polaritons (SPP), but particularly in the terahertz range and below it, it is bound to the structure also by means of its structuring. Such a wave is known as \textit{spoof surface plasmons}.  % todo cite
The notch in reflectance is always observed at the exact frequency that stipulates that the plasmon wavelength is equal to the unit cell size $a_x$, and has a direct connection with the \textit{Wood's anomaly} \cite{wood1902remarkable},  observed in 1902 as an abrupt drop in reflectance on a metallic grating.

Since the structure and fields are symmetric with regards to the $x$-axis, an equal amplitude of plasmons propagating forward and backward is excited. They form a standing plasmonic wave, with points of zero oscillating current (nodes) centered at the slit centers. The extraordinary transmission relies on coupling of the incoming plane wave to SPP, coupling of SPP to their mirror counterpart on the rear side of the structure, and re-radiation of the plane wave. 

Surface-plasmon assisted transmission through a slit was studied earlier and optimum slit width for wave-plasmon coupling was found at $d_x \approx 0.23 \lambda$ \cite{lalanne2005theory}, which matches the lowest-frequency point of the zero-reflection curve in Fig. \ref{fg_yslit_xscan}a. The coupling strength is however not crucial for extraordinary transmission efficiency; since it is a resonant process, it can transmit high amplitude by weak coupling through arbitrarily narrow slits. 
...
For this to happen, however, extraordinary transmission requires strict periodicity of the slits along the $x$-axis, and symmetry of the front and rear sides of the structure along the $z$-axis. A numerical experiment (not shown here) has proven that the $|r| \sim 0$ notch disappears when a dielectric substrate approaches one side of the structure. The dielectric did not have to touch the metal directly; a distance similar the approximate decay length of the surface plasmons or similar to $a_x$, was sufficient. When the dielectric substrate was added symmetrically from both sides, the extraordinary transmission was restored.
...
The current-driven homogenisation setup simulates one unit cell in an exact periodic environment of other cells, and is fully  suitable even for structures with strong coupling of neighbouring cells. 

%TODO  add CDH results for Yslits of different cell spacing: 
...
A natural modification of the above discussed structure is to add conductive connections between the metallic stripes. Transmission through perforated sheets was studied in 1907 by Rayleigh \cite{rayleigh1907dynamical} % todo with the main conclusion that.. 
and half a century later, its resonant behaviour in the metallic mesh has been later used as a microwave and far-infrared filter \cite{ulrich1967effective,ulrich1967far}. 

Already in 1950s the sheets were proposed to be periodically arranged into an artificial dielectric \cite[p. 58]{brown1953artificial} with index of refraction lower than 1, but the field propagated parallel to the slabs, and the operation of the structure was different than what is employed nowadays.  % http://photonics.inescporto.pt/links/fct2013-1/metap14.pdf
	
	has been known for half a century \cite{vogel1964transmission}
	... cite ebbesen?  %Babinet principle?
	... EOT transmission --- Aperture-array transmission --- Wood anomaly --- standing surface plasmons
	... appears hard to be homogenized by the s-parameters \cite{wang2010composite} -- establish why
...
Although fishnets appear to have nothing in common with other negative-index structures, their interaction with electromagnetic wave in fact bears a close resemblance to these: The conventional explanation of the negative refractive index of fishnets is based on the combination of negative effective permittivity and permeability. 

The former, $\eeff'<0$ should result from the inductive effect of the metallic connections, acting similar to the wires oriented along the $x-$axis.  
The fishnet metamaterial is highly anisotropic, and the effective index of refraction is valid roughly within 0.1 rad from the axis, as was noted in the early study on artificial dielectrics \cite{brown1953artificial}. Under higher angles, jelaborated e.g.  index of refraction 

The latter should be formed by individual resonances localised close to the fishnet holes, which exhibit a magnetic dipole moment.


\paragraph{Negative effective index of refraction}%{{{
\add{
...
\end{figure}
%}}}
\paragraph{Quadrupole resonance}%{{{
% TODO 
