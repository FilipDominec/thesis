...
Unlike split-ring resonators, where the high-frequency dissipative losses depend on many factors such as the eddy currents (skin effect) and details in the geometry, the losses in dielectric resonators are well defined and their effect can be reliably computed by the FDTD simulation. 
...
The actual permittivity of the experimentally measured samples depended on their preparation, in particular the volume fraction of microscopic voids. The model in \cite{baumard1977_epsilon_TiO2} gave the ratio of $\epsrl''/\epsrl'$ only, but since $\epsrl'$ could be determined from the resonance frequencies in the experimental spectra, the corresponding imaginary part could be easily computed.

The reflectance, transmittance and effective parameters of rutile spheres with radius $\rho=15\;\upmu$m is shown in Fig. \ref{fg_Spheres_lossscan}. Three different levels of losses are shown, with the realistic value represented as 100 \%, and accompanied with a hypothetic dielectric with 10\% and 1\% losses. 

Most importantly, the losses smoothen the spectra of all individual resonances. The magnetic resonance is located at 530 GHz and the electric one at 790 GHz, the second magnetic resonance is at 1040 GHz. As a natural result of the Lorentzian model, the dielectric losses grow proportional to the frequency.

At the right hand side of the plot, for $f \sim a/(2c) \approx 1.5$ THz, the structure reaches a Bragg band gap, which is not appreciably affected by the amount of losses since most of the field energy in the Bragg resonance is concentrated in free space between the particles.
