...
% TODO discuss the PWEM results, illustrate folding
...
\begin{figure}[t] \caption{Amplitude of reflectance, transmittance and retrieved effective index of refraction for the dielectric slab of 10\% fill fraction and varied permittivity $\epsrl \in \{4,9,16\}$} \label{fg_Slab_fillfraction01_epsilon_comparison} \centering 
\begin{tabular}{r}
\includegraphics[width=0.85\textwidth]{img-meep/Slab_fillfraction01_epsilon_comparison_r.pdf}\vspace{-10.5mm}\\
\includegraphics[width=0.85\textwidth]{img-meep/Slab_fillfraction01_epsilon_comparison_t.pdf}\vspace{-10.5mm}\\
\includegraphics[width=0.85\textwidth]{img-meep/Slab_fillfraction01_epsilon_comparison_nr.pdf}\vspace{-10.5mm}\\
\includegraphics[width=0.85\textwidth]{img-meep/Slab_fillfraction01_epsilon_comparison_ni.pdf}\vspace{-5mm}\\
\end{tabular}
\end{figure}
...
Fig. \ref{fg_1dbd}a shows the folded dispersion curves for a plane wave propagating in vacuum on which we imposed virtual periodicity.
To save space, the Bloch theorem was used to plot the dispersion curves as \textit{folded}, but one can easily imagine how the curve unfolds into a linear dispersion of vacuum, know as the.\textit{light line}. No scattering occurs for homogeneous vacuum, thus for any frequency $f$ exists a real wavenumber $k$ corresponding to a propagating wave. 

Fig. \ref{fg_1dbd}b is obtained by introducing periodic dielectric layers. \textit{Band gaps} of nonzero width separate the allowed bands, coresponding to frequency ranges where the wave can not propagate through the structure. The first photonic band gap starts at the frequency where half wavelength matches the cell spacing, i.e. when $2\pi / K= a/2$ as is depicted in the subplot \textit{X1} of Fig. \ref{fg_1dbd}b. The next band starts at the same wavenumber $K$, but the wave now has higher frequency because in the subplot \textit{X2} of Fig. \ref{fg_1dbd}b it is shifted by half its period so that it maximizes the electric field energy that is localized in the areas of lower permittivity. Higher Bragg band gaps are formed by the same mechanism.
...
The reflection amplitude of a lossless dielectric slab can become zero under a  condition for  states that if an integer number of the halfwaves fits into either of the dielectric layers. Unlike the Bragg resonance, this effect can be observed even in a single isolated unit cell; in fact it is the well known \textit{Fabry-Pérot resonance}. A special case can occur when both Bragg and Fabry-Pérot conditions are fulfilled simultaneously; then a zero-width band gap is obtained and two photonic bands are adjacent to each other just as they were in vacuum (Fig. \ref{fg_1dbd}a)

An example class of structures that exhibit zero band-gap width are binary 1-D PhCs with equal optical thickness of both slabs. Exactly at the frequency where a whole wavelength should fit into the unit cell, thus forming the second band gap, the scattered amplitude from each of the layers becomes zero. This special condition \textit{gap-less} transition between photonic bands.
...
The structure formed from a regular square lattice of conductive wires exhibits the most interesting properties when the electric field is parallel to the wires. This polarisation will be the assumed in the following. 
...
%====================================================================================================
\section{Metallic sheet with slits} % references to ->
