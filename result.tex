...
The structure composed of alternating dielectric layers, effectively a one-dimensional photonic crystal, is the simplest periodic structure to be started with. It has been investigated thoroughly in the previous century and found major application as the dielectric mirror. 

Thanks to its continuous translational symmetry in the transverse direction, it exhibits only a subset of all different phenomena that can be observed in most periodic structures. The key difference is that no \textit{individual resonances} can occur. This does not imply that the energy would have to be distributed evenly in the structure, but that energy is stored and transported solely by propagating, non-evanescent waves.

% for comparison with the structures presented in the following. 

% TODO discuss the PWEM results, illustrate folding


% TODO N x half-wave in unit cell --> bragg gap,   N x half-wave in either of the layers  -->   
%		e.g. one example is when the layers have the same  (optical thickness, $n d$), the Bragg band-gap width is zero
...
\begin{figure}[ht] \caption{Dispersion curves for a one-dimensional photonic crystal, computed using PWEM. The side plots show the shape of the fields in the $(y,z)$ plane, at the frequencies of the band edges. Note that each field plot spans two unit cells. The electric field $E_x$ is plot as color map. \textbf{a)} In free space, \textbf{b)} in dielectric layers with permittivity $\varepsilon = 12$ and 15 \% thickness (outlined by black lines)} \label{fg_1dbd} \centering 
...
% Or put everything in the theory section?
...
% TODO note the ripples inside the band-gap (for more cells)

% TODO Finite planar structure with defect mode {skoromets2013}
