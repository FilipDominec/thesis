...
% todo negative index structure, ref to Fig. 2.11

%}}}
\paragraph{Variants of split-ring resonators}%{{{
\begin{figure}[b] \caption{Other variants of split-ring resonators: \textbf{(a)} double SRR, \textbf{(b)} symmetric version thereof, \textbf{(c)} double SRR with cross-connection between the inner and outer rings, \textbf{(d)} illustration of a square resonator} \label{fg_emcSRR_resonances} \centering 
...
\begin{overpic}[height=0.22\textwidth]{img/drawing_dSRRcrossed.pdf}\put (1,81) {\textbf{(c)}}\end{overpic}\quad
\begin{overpic}[height=0.22\textwidth]{img/drawing_dSRRsquare.pdf} \put (-6,81) {\textbf{(d)}}\end{overpic}
\end{figure}
Diverse metamaterial designs based on the SRR principle were developed since the apparently original publication from Pendry et al. from 1999 \cite{pendry1999magnetism}.
Many SRR designs involve a smaller ring nested inside the original one, as shown in Fig. \ref{fg_dSRR}a,b in the asymmetric and symmetric variants. The long narrow gap between the inner and outer split ring enhances their capacitive coupling. This way, the frequency of the magnetic resonance can be reduced without increase of the overall SRR dimensions. Such designs usually do not need capacitor pads, and can be made by etching on microlitography. 

A modification of this \textit{double split-ring} design includes cross-connection between the inner and outer conductors as in Fig. \ref{fg_dSRR}c; this version also breaks mirror symmetry. 

The square "ring" resonator (Fig. \ref{fg_dSRR}d) is also widely used, without any substantial difference from the round SRR.


All SRR structures in Fig. \ref{fg_dSRR} can be made flat in the plane perpendicular to the magnetic field, which facilitates their fabrication. At terahertz and higher frequencies, using a thin metal film should not be much detrimental to the SRR conductivity, since the skin depth caused by eddy currents is often submicroscopic \cite{gibbons2010scalable}.

On the contrary, a similar way of operation is achieved by structures infinite in the direction of the magnetic field. This allows to fabricate magnetic resonators by rolling stripes of a semi-metallised plastic foil, producing a \textit{swiss-roll} metamaterial \cite{gibbons2010scalable}, or by partially sputter-coating a nonconductive fibre by metal. Such structures however seem to exhibit relatively high losses when applied above the microwave range.

Metallic \textit{cut-wire pair}, or also \textit{strip pair}, metamaterial was designed for operation in the infrared or optical range. It can be understood as a split-ring resonator elongated in the direction of the electric field with splittings 

clusters of metallic microspheres
 %note swissroll metamaterial: Gibbons2009 , Wang2010-Fiber_MMs_with_negative_permeability.pdf
...
%\end{figure}
%}}}
% TODO \paragraph{Fano resonances}%{{{
...
Already in Eq. (\ref{}) it was assumed that at higher frequencies, there is no difference in the effect of conduction and polarisation currents. The current conducted around a split-ring resonator has a direct analogy in the polarisation current circulating around an axis of a dielectric sphere. The capacitor in the SRR is then replaced by the distributed capacitance of the whole dielectric. The magnetic dipole moment created by the polarisation current goes through the axis of the sphere. The resonance frequency is inversely proportional to the radius $\rho$ of the sphere, and in general, decreases monotonously with the growth of the dielectric permittivity $\epsrl$.

The second resonance has an electric dipole moment. It is similar to the magnetic one except for that now the magnetic field circulates around the axis of the sphere, and the polarisation current forms a dipole. Although the topology of both resonances is the same, the resonance frequency of the magnetic one is lower, since the electric field is then almost entirely confined to the dielectric volume, whereas for the electric resonance, most of the streamlines of the electric field pass through the surrounding air
...
The Mie theory was developed for single particles in vacuum, but for high enough dielectric permittivity $\epsrl \gtrsim 50$ the resonant fields are well confined in the dielectric volume and are not much affected by the presence of the neighbouring cells.
