...
Obviously,  in both Eqs. (\ref{eq_freqM1_rhoxyz}) and (\ref{eq_freqE1_rhoxyz}), the three exponents must sum to -1 so that they are compatible with the scaling rule for a sphere in Eq. (\label{eq_freq_rho}). 
These values are approximate only, and were interpolated from batches of simulations which scanned through the ellipsoid parameters.

When the ellipsoid axes are oriented randomly, the resonance frequencies remain almost the same; also the resonant modes are  preserved with respect to the ellipsoid orientation. An incident wave in such a case can excite multiple non-degenerate magnetic or electric Mie resonances at once. The structure then changes the polarisation state of the wave, which is beyond the scope of this analysis.
