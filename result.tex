...
For easier interpretation, the peculiar interplay of the individual resonances and the band gaps is redrawn in Fig. \ref{fg_drawn100}, where each photonic band gap is represented by a coloured area and the individual resonances by thick black lines. The small black patch between the first and second band gaps covers the range conditions necessary for reaching $\Neff'<0$, provided that the dielectric is titanium dioxide with permittivity $\epsrl \approx 92$. 

In accordance with the previous discussion, the electric and magnetic Mie resonances approach each other when the unit cell size $a$ decreases from 120 to 90 $\upmu$m, and eventually they meet and vanish for $a \lesssim 85$ $\upmu$m, forming an "U"-shaped curve. The choice of $a$ as the scanning parameter enables to clearly illustrate this phenomenon; on the contrary, the frequencies of the band gaps roughly follow the $1/a$ curves. The central frequency and width are strongly influenced by the individual resonances, in order to satisfy the rule of each individual resonance being "dressed" in a band gap.

In the  with low-enough losses, the index of refraction grows as a function of frequency in all photonic bands. From the beginning to the end of each photonic band, it gains a difference of one Brillouin zone. The only way how its value decreases is by means of an individual resonance. A simple arithmetic rule for reaching negative index of refraction $\Neff'(f_1)$, at a given frequency $f_1$, is therefore that the number of photonic band gaps counted from $f=0$ to $f_1$ must be strictly less than the number of individual resonances. 
...
This heuristics eliminates the possibility of attaining $\Neff'<0$ from any higher-order resonances, although they are clearly present in Figs. \ref{fg_spacingscan100} and \ref{fg_drawn100} and they even persist for smaller unit cell sizes.

%}}}
\begin{figure}[h!] % fg_fxplot %{{{
\begin{minipage}[c]{0.51\textwidth}
	\hspace{-2mm}\begin{overpic}[width=.99\textwidth]{img-meep/erods_fxplot_cs120e-6.pdf} 
		\put(110,8){\includegraphics[width=.5\textwidth]{img/ERods_sketch_recordedline.pdf}}
		\put(7,90) {\textbf{(a)}} 
	\end{overpic}\\
\end{minipage}
\begin{minipage}[c]{0.49\textwidth}
	\caption{Spatially resolved spectra of the electric field for the unit cell cell sizes \textbf{(a)} $a=120$  }\vspace{3cm} \label{fg_fnquadrup}
\end{minipage}  
\vspace{-12mm}
\end{figure} 
\begin{figure}[h!] 
...
To explain the disappearance of Mie resonances, we needed to inspect the fields inside the structure. This required a new kind of plots showing how the electric field amplitude depends simultaneously on the position along the wave vector (i.e. $z$-axis) and on the frequency (Fig. \ref{fg_fxplot}).  The electric field was recorded along the line between the dielectric rods arranged in three layers; the field amplitude was stored in each simulation step. At the end of the simulation, for each point in space, the spectrum was computed and the magnitude of $|E_x(z, f)|$ was plotted on the horizontal axis. The color map was chosen as logarithmic with dark blue representing lowest field magnitude and white highest, differing by 5 orders of magnitude. 
To the author's knowledge, such kind of plots have not been published in any paper yet. 

The case of sparse rods (Fig. \ref{fg_fxplot}a) clearly illustrates that from the lower edge of the first band gap, at 0.42 THz, to the electric resonance at 0.745 THz, one nodal plane exists in each unit cell, introducing a $+\pi$ shift of the field phase between adjacent cells. At a nodal plane, the field intensity drops to very low values. 
While the field pattern close to the resonant frequency is confused by finite dimensions of the three-cell layer, above the resonance it clearly demonstrates the exponential decay of the evanescent wave. In this region, the electric dipole of the rods is opposite to the surrounding field, but the nodal planes form closed surfaces around each rod, and the phase difference between the unit cells is zero. From 0.97 THz, the second photonic band starts which allows the electromagnetic energy propagate through the structure.

The second case of medium-density rod array (Fig. \ref{fg_fxplot}b) is known to bring both resonances into the first band gap. At the frequency of the first resonance, the nodal plane in each unit cell is removed. In contrast to the previous plot, the second resonance further reduces the phase difference between the unit cells to negative value $-\pi$. Faint nodal planes can be observed again in the upper part of the first band gap. 

Finally, 

%}}}
\paragraph{The cause of the Mie resonances vanishing}%{{{
The reason behind the two Mie resonances vanishing is in the change of the nodal surface topology. When the rods are far from each other ($a\gtrsim 88$~$\upmu$m), the individual Mie resonances create closed regions delimited by a nearly circular nodal surface, dominated by the near field of the resonance dominates. 

Upon reducing the unit-cell size ($a\lesssim 85$~$\upmu$m), the regions of opposite fields start to overlap with those from the neighboring cells and the nodal surfaces interconnect with each other. The wave propagating through an unit cell then changes  its sign twice, which manifests itself as a $2\pi$ phase change.  % TODO reformulate
...
\add{
%which imposes strict limits on the geometry, 
