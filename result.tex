...
A very similar structure Ref. \cite{vynck2009all} is studied within the metamaterial paradigm. Each rod is viewed as a separate dielectric resonator, with analytical formula for the Mie resonance frequencies. The overlap of the electric and magnetic resonance is predicted even for a relatively low permittivity $\epsrl = 12$. Based on this, its authors conclude that a metamaterial with $\Neff'<0$ can be built of silicon rods -- which is in direct contradiction with our results presented above.

Ref. \cite{peng2007} experimentally investigates a metamaterial made of square bars parallel to the electric field, with very high permittivity of $\epsrl=600$. The geometry of the bars is equivalent to the \textit{dense array} of cylindrical rods $a/\rho \sim 3$, which means it should classify as the \textit{dense} array, exhibiting no individual resonances and no negative index of refraction. However, as the major result of the paper \cite[Fig. 3bc]{peng2007} it is demonstrated that negative refraction occurs at the wedge, and it is more or less maintained upon randomization of the rod positions.

Our results \cite{dominec2014transition} appear to be in contradiction with the predicted \cite{vynck2009all} and observed \cite{peng2007} negative refraction.


\cite{rybin2014photonic}
\cite{rybin2015phase}
