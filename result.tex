...
%}}}
\begin{figure}[htb] %{{{ fg_yslit_xscan %% TODO REPLACE WITH HR
...
At a frequency just below the onset of diffraction, a relatively narrow notch in the reflectance spectrum is observed. The frequency where $|r|$ drops close to zero depends on the slab thickness $d_z$ -- which will be shown later -- and on the slit width $d_x$. In the above figures where $d_z$ was fixed at $20$ $\upmu$m, the lowest frequency of the notch around $2.8$ THz is reached for $d_x/a_x \sim 0.3$ (see Fig. \ref{fg_yslit_xscan}a). For a narrower or wider slit, the frequency of the notch increases and eventually approaches the diffraction limit of $3.0$ THz. 

Nonetheless, the notch does not disappear and always some frequency can be found where nearly 100 \% of the energy is transmitted through the structure, even when the slits are extremely narrow.

The phenomenon known as \textit{extraordinary transmission} is mediated by surface plasmons propagating along the front and rear surfaces of the metallic sheet. The notch in reflectance is always observed at the exact frequency that stipulates that the plasmon wavelength is equal to the unit cell size $a_x$.

% todo ... counter-propagating surface plasmons, with their wavelength exactly matching the transverse unit cell dimension $a_x$.
Since the structure and fields are symmetric with regards to the $x$-axis, an equal amplitude of plasmons propagating forward and backward is excited. They form a standing plasmonic wave, with nodes of the oscillating current centered at the slit centers. The extraordinary transmission relies on coupling of the incoming plane wave to SPP, coupling of SPP to their mirror counterpart on the rear side of the structure, and re-radiation of the plane wave. Since it is a resonant process, it can transmit high amplitude through weak coupling of arbitrarily narrow slits. 

For this to happen, extraordinary transmission however requires strict periodicity of the slits along the $x$-axis, and symmetry of the front and rear sides of the structure along the $z$-axis.
A numerical experiment (not shown here) has proven that the $|r| \sim 0$ notch disappears when a dielectric substrate approaches one side of the structure. The dielectric did not have to touch the metal directly; a distance similar to $a_x$, or, the approximate decay length of the surface plasmons, was sufficient. When the dielectric substrate was added symmetrically from both sides, the extraordinary transmission was restored.
...
So far we discussed the properties of a single layer only, but when the unit cell size along the wave propagation axis is given, the effective parameters can be also computed for an infinite stack thereof.

However, due attention must be paid to whether the conventional scattering parameter method gives correct results, since this is one of structures with strong coupling to the neigbouring cells. A great portion  of the resonant energy is carried by the surface plasmons that are not localized inside the gap. On the contary, it was already shown that SPP easily couple with those on nearby metallic surfaces, forming slot-waveguide modes \cite{weiner2011electromagnetics}.
...
%}}}
\cite{lalanne2005theory} %Lalanne-Theory_of_fishnet_optical_NIM.pdf
...
\cite{weiner2011electromagnetics} % Weiner-Transmission_through_metallic_slits.pdf
...
The points of low reflectance in Fig. \ref{fg_yslit_dscan}a correspond to the effective impedance of the structure $\Zeff \approx 1$, i.e. for the structure being matched to the surrounding air. The first set of such points starts very close to 3 THz (the diffraction limit)
such a point in the spectra corresponds to a fundamental resonance of the slit.
% todo for a slit in a  thicker metallic slab, the 
...
% experimental negative refraction of a wedge, dgedge.operated around 1.5 um
