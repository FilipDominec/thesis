...
Dispersion curves for a one-dimensional photonic crystal, computed using PWEM, are shown in Fig. \ref{fg_1dbd}. The side plots show the shapes of the electric field in the $(y,z)$ plane,  at the respective frequencies of the band edges. The electric field was chosen to be polarized along the $x$-axis and is plotted as a color map (white corresponding to zero). For better readability, each field plot spans two unit cells. 

% todo
Note that with the correct branch retrieval, the index of refraction never drops with frequency, except for a photonic band gap; this would otherwise imply the group velocity would be higher than the phase velocity.

% TODO draw and append axes+colorbar to the field distribution plots
% TODO visualise the permittivity, but how?

Fig. \ref{fg_1dbd}a shows the folded dispersion curves for a plane wave propagating in vacuum.
