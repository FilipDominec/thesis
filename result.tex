...
%}}}
\begin{figure}[h!] %fg_Spheres_lossscan %{{{
	\caption{Amplitude of \textbf{(a)} reflectance, \textbf{(b)} transmittance and \textbf{(c)} effective index of refraction $\Neff = \Neff' + \ii \Neff''$ of TiO$_{2}$ spheres with varied loss compared to the natural one from Ref. \cite{baumard1977_epsilon_TiO2}; sphere radius $\rho = 15 \upmu$m, unit cell size $a=100$ $\upmu$m.} \label{fg_Spheres_lossscan} \centering \vspace{-0.0\textwidth} 
\begin{tabular}{r}
...
For the preparation of the fishnet spectra and this plot,
the resolution in the FDTD simulation was set to be very coarse with a voxel dimension of 10 $\mu$m. The discretisation error has broken the mirror symmetry of the fishnet hole (see Fig. \ref{fg_expe_fishnets}), and allowed a nonzero coupling of the predominantly quadrupole mode to the propagating wave, thus enabling to explain the experimentally observed resonances around 880 and 710 GHz in Fig. \ref{fg_expe_fishnets}.  
