...
Its left panel shows the folded dispersion curves for a plane wave propagating in vacuum on which we imposed virtual periodicity. To save space, the dispersion curves were plotted as \textit{folded}, but one can easily imagine how the curve unfolds into a linear dispersion of vacuum, known as the \textit{light line}. No scattering occurs for homogeneous vacuum, thus for any frequency $f$ exists a real wavenumber $k$ corresponding to a propagating wave and there are no band gaps of nonzero width. 
...
Dielectric rods aligned parallel to the magnetic field exhibit spectra similar to the sphere arrays. % TODO ref fig
...
% note the spatial dispersion is negligible
...
The discussion of the previous structure has shown that the choice of permittivity and geometry obviously leads to a qualitative change in the structure behaviour. This becomes even more important in the case of dielectric rods parallel to the electric field. 

Fig. \ref{fg_erods_acomparison} shows the spectra of effective parameters for three rod arrays of the same permittivity $\epsrl(f=1\text{ THz}) = 89.5+0.23\ii$ and diameter $\rho=10$ $\upmu$m. They only differ by the unit cell size $a \in \{80, 95, 120\}$ $\upmu$m, yet such a structure obviously behaves in several different ways depending on $a$. Moreover, not all differences may be directly seen from the PWEM results, as was the case in Fig. \ref{fg_rodh}.  % todo rewrite?
...
%\cite{yang2015}
Starting with the blue curves in Fig. \ref{fg_erods_acomparison} representing a sparse array with $a=12\rho = 120$ $\upmu$m, one can clearly identify two individual resonances around 745 and 1170 GHz, each introducing a familiar shape in the spectra of $\Neff(f)$. From the effective permittivity $\eeff(f)$ and permeability $\meff(f)$, the first resonance can be identified as an electric one, and the second as a magnetic one. The electric resonance has a very strong dipole, compared to any of resonances shown previously in Fig. \ref{fg_rodsphere_comparison}; the fundamental asymmetry arises from the fact that the spheres and rods are made of a dielectric that primarily interacts with the electric field, while its permeability $\murl=1$. 

In spite of the strength of the first resonance, the corresponding region of $\eeff'(f)<0$ does not overlap with that of $\meff'(f)<0$ for the sparse rod array with $a=120$ $\upmu$m and no negative index of refraction is obtained. This behaviour is analogous to that of the electro-magnetic resonator possessing too high a capacitor radius $\rho_c\gtrsim 18$ $\upmu$m in Fig. \ref{fg_emcSRR_icr}.

%}}}
\paragraph{Effect of inter-cell coupling}%{{{
By increasing the filling fraction of the dielectric rod, the electric and magnetic resonance frequency can be efficiently tuned compared to each other. Fig. \ref{fg_sketchfield} illustrates the reason why this is possible, which can be traced down to the symmetry of the magnetic field. 

The electric resonance, with the electric dipole pointing along the rod, and the magnetic field circulating around it, introduces opposite orientation of the magnetic field at the cell boundaries. Reduction of the cell size $a$ also reduces the effective inductance of the rod per unit length, thus shifting the electric resonance upwards.

The magnetic field surrounding the rod near the magnetic Mie resonance, on the contrary, has the same orientation at the cell boundaries. Thus the reduction of $a$ contributes to the inter-cell coupling, and reduces the magnetic resonance frequency.

Note that this strong coupling is specific for the rod orientation parallel to the electric field. In the previous chapter concerned with the rods oriented parallel to the magnetic field, the electric field was mostly confined to the dielectric volume, and the coupling effect was thus weaker. The resonance frequencies of dielectric rods parallel to the magnetic field can not be tuned in a similar manner.

%}}}
\begin{figure}  %fg_sketchfield%{{{
\caption{Cross-sections of the dielectric rod the first two resonant modes. The red and blue color shows positive and negative values of the $E_x$ component of the electric field, while the magnetic field is represented by the arrows.  \textbf{(a)} In the electric Mie resonance, the magnetic field circulates around the rod, and the electric field filling the whole cross-section of the rod. \textbf{(b)} The magnetic Mie resonance with the magnetic field crossing the center of the rod, and electric field having opposite polarity in the front and rear portions thereof. } \label{fg_sketchfield} 
\centering \includegraphics[width=.7\textwidth]{img/ERods_1st_and_2nd_Mie_resonance.pdf}
\end{figure}
%}}}
...
Through moderate reduction of the unit cell size $a$ to 95 $\upmu$m, as represented by the green line in Fig. \ref{fg_erods_acomparison}, the electric and magnetic resonances frequencies approach each other, shifting to $f_M \approx$950 and $f_E \approx$1070 GHz, respectively. This eventually leads to an overlap of the $\eeff'(f)<0$ and $\meff'(f)<0$ regions, and a negative-index band is formed between 1130 and 1200~GHz.  

With reduction of $a$, the points of near-zero transmittance $f_E$ and $f_M$, indicating individual resonances in Fig.   come closer to each other. The maximum amplitude transmittance between drops approximately with the second power:
\begin{equation} \underset{f \in\,\langle f_E,f_M\rangle}{\text{max}} |t| \propto \frac{1}{(f_E-f_M)^{2}}, \label{eq_fEfM}\end{equation}
and the transmitted energy drops with the fourth power of the resonance frequency difference, $f_E-f_M$. Simultaneously, the transmittance reaches almost 100 \% relatively close to this low-transmittance region. Single layer of dielectric rods can thus be used as a thin, yet extremely efficient filter with much better extinction ratio than the Fabry-Pérot resonances in a dielectric slab can achieve (see Fig. \ref{fg_Slab_fillfraction015_epsilon_comparison}).

%Above the negative-index band, 

%}}}
\paragraph{Dense rod array}%{{{
Further reduction of the unit cell size $a$ to 80 $\upmu$m does not lead to a cross-over of the resonance frequencies, as might be expected. Instead, the individual resonances disappear, and an ordinary Bragg band gap remains spanning from 480 to 1150~GHz. The corresponding red lines do not exhibit any zero-transmittance points (in Fig. \ref{fg_erods_acomparison}b), nor any fast drop of $\Neff'(f)$ (see Fig. \ref{fg_erods_acomparison}c). The effective permittivity and permeability cease to have physically useful values above the middle of the first photonic band.

The higher portion of the $\Neff(f)$ spectra for $a=80$ $\upmu$m still seems to approximately copy that of $a=95$ $\upmu$m or even $a=120$ $\upmu$m: the negative-index band (between the -1th to the 0th Brillouin-zone boundaries) on the green curve has its analogy in a similarly narrow band on the red curve (between the 1th to the 2nd Brillouin-zone boundaries). Also the higher frequency features tend to be similar. This suggests that the major impact of merging of the resonances is in \textit{changing the wave phase} by $2\pi$ per unit cell.
%}}}
\paragraph{Continuous scan through the unit cell sizes}%{{{
Since the Maxwell equations are scale-invariant, the structure under study is fully described by the ratio of radius to the unit cell size $\rho/a$ and the dielectric permittivity $\epsrl$. Scanning through $\rho$ would tune the Mie resonances would be tuned, but the Bragg band gaps frequencies would remain almost unchanged. 
The impact of $a$ to the Mie resonances is however more important for the discussion of the regimes of operation described above. We present a parametric scan through the unit cell size $a\in\langle20,200\rangle$ $\upmu$m in Fig. \ref{fg_} Thus, the Bragg band gaps tend to follow the $f = mc/(2a)$ hyperbolas, while the Mie resonance frequencies are roughly constant for $a>150$ $\upmu$m.

As expected, the Mie resonances start to deviate from their vertical asymptotes for smaller unit-cell sizes; %todo
%}}}


\paragraph{What causes the $2\pi$ change in refractive index?}%{{{

% from our paper:    Another case of interest where evanescent waves are obtained is that of $\Neff' = 0$ (center of the first Brillouin zone, $q=0$) and $\Neff''>0$. Here the waves exponentially decay in the medium without any phase change. However, this behavior has a different origin: it is connected to a plasma-like response of the material ($\varepsilon<0$ or $\mu<0$) and in this paper we use the term \textit{plasma band gap} to refer to it.
...
\cite{peng2007, vynck2009all}

%todo discuss from short.pdf: "In photonic bands, no matter if $N_{\text{eff}} > 0$  or $N_{\text{eff}} < 0$, the real part of $N_{\text{eff}}$ always grows in accordance with the Foster theorem:
...
% todo introduce "plasma band gap"

% todo We conjecture that the presence of two Mie resonances in the \textit{first} combined band gap is a necessary and sufficient condition for $\Neff'<0$ to occur.

\begin{figure}%{{{  spacingscan100  TODO REPLOT for TiO2
...
In the upper-right corner of both plots, for $a>c/f$, an empty area is left where the diffraction precludes the determination of effective parameters. 
} \centering 
...
% \begin{figure} OFF fg_spacingscan100formation \centering %{{{
% \includegraphics[width=0.5\textwidth]{img/ERods_sketch_of_separate_spectra_to_continuous_scan.pdf}
% \caption{} \label{fg_spacingscan100formation}.  % \end{figure} %}}}
\paragraph{Effect of the dielectric permittivity}%{{{
% todo Comparing the low-permittivity and high-permittivity cases in Fig. \ref{fg_ebar_radiusscan}a,b, one can see there exists some minimum limit for the permittivity contrast that is necessary for both electric and magnetic resonances to be located in the first gap simultaneously. We determined this scale-invariant value to be roughly 80.

%}}}
\begin{figure}%{{{ fg_drawn030
	\centering
...
The behavior described above is confirmed by the plots of the complex index of refraction $\Neff',\: \Neff''$ for a continuously varying unit cell size $a$ from 20 to 200~$\upmu$m (Fig.~\ref{fg_spacingscan100}). Here, again, the constant values of the dielectric permittivity $\varepsilon_r=100$ and the rod radius $\rho=10$ $\upmu$m are used. 
