...
\item{The effective permittivity $\eeff$ is the weighted average of the constituent media, which determines the refractive index:
...
The structure formed of a regular square lattice of conductive wires exhibits more interesting properties when the electric field is parallel to the wires, and this polarisation will be assumed in the following. The lattice of wires perpendicular to the electric field does not appreciably interact with the electromagnetic wave, until the wire thickness is of similar magnitude to their spacing; such a case behaves in a substantially different way and is discussed later (see Section \ref{section_eot}).

In the high-frequency part of the spectrum above the first photonic band, where more than a half wave fits into the unit cell, the layer of metallic rods can be approximated as a thin scattering slab from the previous section. The high-frequency part of the spectrum consists of photonic bands interlaced with Bragg band gaps. In contrast with the dielectric PhC described aboveno Fabry-Pérot resonances are observed and the band-gap width reduces monotonously with growing frequency.

\begin{figure}[b] \caption{Amplitude of \textbf{(a)}  reflectance, \textbf{(b)} transmittance \textbf{(c)} effective index of refraction $\Neff$ and \textbf{(d)} effective permittivity $\eeff$ for the array of wires made of gold, depending on the wire radius $\rho_w \in \{1, 2, 4, 8, 16\}$ $\upmu$m with a fixed unit cell size $a 100 \upmu$m. } \label{fg_Slab_fillfraction015_wireradius_scan} \centering \vspace{-3mm}
\begin{tabular}{r}
...
The low-frequency behaviour of wire array, differs from that of a 1-D PhC by that, from zero frequency up to some \textit{effective plasma frequency} $f_p$, the array exhibits a band gap where $\Neff$ is pure imaginary. As a result of $K\approx 0$, in the low-frequency part of the spectrum, the effective permittivity is a physically meaningful quantity and follows the 
\begin{equation} \eeff(f) = 1 - \frac{f^{2}}{f_p^{2}}\label{eq_eeff_plasma}\end{equation}
law typical of inductive media, such as metals or plasma. The physical origin of $\eeff<0$ is however different than in continuous media, where it is due to the effective mass of electrons. In contrast, the negative effective permittivity in wire arrays is mostly due to the magnetic field circulating around the conductor, and therefore does not substantially depend on the internal plasma frequency of the constituent metal.

It does, however, depend on the geometry described by two parameters, the wire radius $\rho_w$ and the unit cell size $a$. The former dependence is illustrated by the spectra of reflectance, transmittance, refractive index and effective permittivity in Fig. \ref{fg_Slab_fillfraction015_wireradius_scan}. The effective plasma frequency $f_p$ can be easily determined as the freqeuncy where $\eeff'(f)$ crosses zero, which coincides with the lower edge of the first photonic band. From the plots, it can be also deduced that $f_p$ is approximately proportional to the logarithm of $\rho_w$.
...
The permittivity is negative for frequencies below its plasma frequency $f_p$, at which $\varepsilon$ gets positive. The value of effective plasma frequency $f_p$ of the wire medium, depending on the wire radius $\rho_w$ and the grid spacing of $a>2\rho_w$, is predicted by the analytic model\cite{maslovski2002wire} as:
