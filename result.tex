...
At frequencies close to $f_p$, but slightly higher, the \textit{phase velocity}\index{velocity!phase v.} in nonmagnetic media 
\begin{equation} v_p = \frac{c}{n} = \frac{c}{\sqrt{\eeff(f)}}\label{eq_vpwire}\end{equation}	
	is proportional to $(f-f_p)^{-1/2}$ and thus very high. Conversely, the \textit{group velocity}\index{velocity!group v.} \cite{mikki2009electromagnetic}
...
One of the simplest examples is formed by bending a wire into a "C"-shaped \textit{split-ring resonator}\index{metamaterial!split-ring resonator} (SRR). To reduce the resonant frequency, capacitor pads can be added to the cut, as shown in Fig. \ref{fg_SRR_types}a. The first resonance in its spectrum has a dominant magnetic dipole moment, and will be denoted simply as the \textit{magnetic resonance}\index{resonance!individual magnetic}. The electric current flows through the wire along a circular path, while the magnetic field has a toroidal shape around the wire.   
